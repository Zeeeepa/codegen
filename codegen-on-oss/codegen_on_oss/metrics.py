...
class CodeMetrics:
    ...
    def complexity_metrics(self) -> dict[str, Any]:
        ...
        for func in callables:
            ...
            complexities.append({"name": func.name, "complexity": complexity, "rank": cc_rank(complexity)})

        avg_complexity = sum(item["complexity"] for item in complexities) / len(complexities) if complexities else 0

        self._complexity_metrics = {"average": avg_complexity, "rank": cc_rank(avg_complexity), "functions": complexities}
    ...
    def line_metrics(self) -> dict[str, Any]:
        ...
        self._line_metrics = {"total": {"loc": total_loc, "lloc": total_lloc, "sloc": total_sloc, "comments": total_comments, "comment_density": total_comment_density}, "files": file_metrics}
    ...
    def maintainability_metrics(self) -> dict[str, Any]:
        ...
        for func in callables:
            ...
            mi_scores.append({"name": func.name, "mi_score": mi_score, "rank": get_maintainability_rank(mi_score)})

        avg_mi = sum(item["mi_score"] for item in mi_scores) / len(mi_scores) if mi_scores else 0

        self._maintainability_metrics = {"average": avg_mi, "rank": get_maintainability_rank(avg_mi), "functions": mi_scores}
    ...
    def inheritance_metrics(self) -> dict[str, Any]:
        ...
        avg_doi = sum(item["doi"] for item in class_metrics) / len(class_metrics) if class_metrics else 0
    ...
    def halstead_metrics(self) -> dict[str, Any]:
        ...
        avg_volume = sum(item["volume"] for item in halstead_metrics) / len(halstead_metrics) if halstead_metrics else 0
        avg_difficulty = sum(item["difficulty"] for item in halstead_metrics) / len(halstead_metrics) if halstead_metrics else 0
        avg_effort = sum(item["effort"] for item in halstead_metrics) / len(halstead_metrics) if halstead_metrics else 0

        self._halstead_metrics = {"average": {"volume": avg_volume, "difficulty": avg_difficulty, "effort": avg_effort}, "functions": halstead_metrics}
    ...
    def find_low_maintainability_functions(self, threshold: int = MAINTAINABILITY_THRESHOLD) -> list[dict[str, Any]]:
    ...
    def find_deep_inheritance_classes(self, threshold: int = INHERITANCE_DEPTH_THRESHOLD) -> list[dict[str, Any]]:
    ...
    def analyze_dependencies(self) -> dict[str, Any]:
        ...
        return {
            "dependency_graph": {"nodes": len(G.nodes), "edges": len(G.edges), "density": nx.density(G)},
            "cycles": len(cycles),
            "most_central_files": sorted([(file, score) for file, score in centrality.items()], key=lambda x: x[1], reverse=True)[:10],
        }
...
class MetricsProfiler:
    ...
    @contextmanager
    def start_profiler(self, name: str, revision: str, language: str | None, logger: "Logger") -> Generator[Any, None, None]:
