...
async def middleware(request: Request, call_next):
    ...
    # Clean up old entries - use a list comprehension to collect expired keys first
    expired_ips = [ip for ip in request_counts if current_time - request_counts[ip]["timestamp"] > RATE_LIMIT_WINDOW]
    ...
    try:
        ...
        # Log the full traceback but don't include it in the response
        # even in debug mode to prevent sensitive information exposure
        return JSONResponse(
            ...
            content={
                ...
                "path": request.url.path,
                "status_code": status_code,
                # Only include a sanitized version of the traceback in debug mode
                "traceback": ["Error details logged on server"] if os.getenv("DEBUG") else None,
            },
        )
