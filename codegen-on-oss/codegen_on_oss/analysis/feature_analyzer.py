...
class FeatureAnalyzer:
    ...
    def analyze_function(self, function_name: str) -> FunctionAnalysisResult:
        ...
        # Get line count
        line_count = (
            len(function.code_block.source.splitlines()) if function.code_block else 0
        )
    ...
    def _analyze_directory(self, directory_path: str) -> FeatureAnalysisResult:
        ...
        for func in self.codebase.functions:
            if (
                hasattr(func, "file")
                and func.file
                and func.file.name.startswith(directory_path)
            ):
                functions.append(func)
        ...
        for cls in self.codebase.classes:
            if (
                hasattr(cls, "file")
                and cls.file
                and cls.file.name.startswith(directory_path)
            ):
                classes.append(cls)
        ...
        for file in files:
            line_count += (
                len(file.source.splitlines()) if hasattr(file, "source") else 0
            )
        ...
        # Calculate average complexity
        avg_complexity = (
            total_complexity / len(function_results) if function_results else 0
        )
