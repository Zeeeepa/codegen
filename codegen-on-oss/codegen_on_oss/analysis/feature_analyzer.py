    ...
    def analyze_function(self, function_name: str) -> FunctionAnalysisResult:
        ...
        for file in self.codebase.files:
            if (
                ...
                and function_short_name.lower() in file.content.lower()
            ):
                is_tested = True
                break
            ...
            for method in cls.methods:
                if hasattr(method, "qualified_name"):
                    try:
                        ...
                        logger.warning(
                            f"Error calculating complexity for method {method.qualified_name}: {e}"
                        )
                ...
                for imp in file.imports:
                    if hasattr(imp, "module_name"):
                        ...
                        for feature_file in feature_files:
                            if (
                                hasattr(feature_file, "module_name")
                                and feature_file.module_name == imp.module_name
                            ):
                                is_external = False
                                break
