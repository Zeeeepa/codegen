...
class FeatureAnalysisResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        """
        return {"feature_path": self.feature_path, "functions": self.functions, "classes": self.classes, "complexity": self.complexity, "dependencies": self.dependencies, "issues": self.issues}
...
class FeatureAnalyzer:
    ...
    def analyze_function(self, function_name: str) -> FunctionAnalysisResult:
        ...
        if not function:
            msg = f"Function {function_name} not found in the codebase"
        ...
        if complexity > 10:
            issues.append({"issue_type": "high_complexity", "severity": "warning", "message": f"Function has high cyclomatic complexity ({complexity})"})

        # Check for long functions
        if line_count > 50:
            issues.append({"issue_type": "long_function", "severity": "warning", "message": f"Function is very long ({line_count} lines)"})

        # Check for many parameters
        if len(parameters) > 5:
            issues.append({"issue_type": "many_parameters", "severity": "warning", "message": f"Function has many parameters ({len(parameters)})"})

        # Check for lack of testing
        if not is_tested:
            issues.append({"issue_type": "not_tested", "severity": "info", "message": "Function does not appear to be tested"})
    ...
    def analyze_class(self, class_name: str) -> dict[str, Any]:
        ...
        if not cls:
            msg = f"Class {class_name} not found in the codebase"
            raise ValueError(msg)
        ...
        if avg_complexity > 10:
            issues.append({"issue_type": "high_complexity", "severity": "warning", "message": f"Class has high average method complexity ({avg_complexity:.2f})"})

        # Check for many methods
        if len(methods) > 20:
            issues.append({"issue_type": "many_methods", "severity": "warning", "message": f"Class has many methods ({len(methods)})"})

        # Check for many attributes
        if len(attributes) > 15:
            issues.append({"issue_type": "many_attributes", "severity": "warning", "message": f"Class has many attributes ({len(attributes)})"})

        # Check for lack of testing
        if not is_tested:
            issues.append({"issue_type": "not_tested", "severity": "info", "message": "Class does not appear to be tested"})
    ...
    def analyze_feature(self, feature_path: str) -> FeatureAnalysisResult:
        ...
        if not feature_files:
            msg = f"Feature {feature_path} not found in the codebase"
            raise ValueError(msg)
        ...
        # Check for high complexity
        if avg_complexity > 10:
            issues.append({"issue_type": "high_complexity", "severity": "warning", "message": f"Feature has high average function complexity ({avg_complexity:.2f})"})

        # Check for many dependencies
        if len(dependencies) > 20:
            issues.append({"issue_type": "many_dependencies", "severity": "warning", "message": f"Feature has many external dependencies ({len(dependencies)})"})

        # Check for untested functions
        untested_functions = [analysis["function_name"] for analysis in function_analyses if not analysis["is_tested"]]
        ...
        return FeatureAnalysisResult(feature_path=feature_path, functions=function_analyses, classes=class_analyses, complexity=complexity_metrics, dependencies=list(dependencies), issues=issues)
