...
class Webhook:
    ...
    def __init__(
        self, webhook_id: str, project_id: str, webhook_url: str, events: list[str], secret: Optional[str] = None, created_at: Optional[datetime] = None, last_triggered: Optional[datetime] = None
    ):
...
class WebhookHandler:
    ...
    def register_webhook(self, project_id: str, webhook_url: str, events: list[str], secret: Optional[str] = None) -> Webhook:
        ...
        webhook = Webhook(webhook_id=webhook_id, project_id=project_id, webhook_url=webhook_url, events=events, secret=secret)
        ...
        if event in self.event_handlers:
            for handler in self.event_handlers[event]:
                try:
                    ...
                    self.logger.exception(f"Error in event handler for {event}: {e}")
    ...
    def _trigger_webhook(self, webhook: Webhook, event: str, payload: dict[str, Any]) -> None:
        ...
        webhook_payload = {"event": event, "project_id": webhook.project_id, "timestamp": datetime.now().isoformat(), "payload": payload}
        ...
        try:
            ...
            self.logger.info(f"Would send webhook to {webhook.webhook_url} with payload: {json.dumps(webhook_payload)}")
        except Exception as e:
            self.logger.exception(f"Error triggering webhook {webhook.webhook_id}: {e}")
