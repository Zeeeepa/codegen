...
class CodeAnalysisClient:
    ...
    def analyze_file(self, repo_url: str, file_path: str) -> dict[str, Any]:
        ...
        Returns:
            Analysis results
        """
        response = requests.post(f"{self.base_url}/analyze_file", json={"repo_url": repo_url, "file_path": file_path})
        response.raise_for_status()
        return response.json()
...
def main() -> None:
    ...
    parser.add_argument("--analyze-file", nargs=2, metavar=("REPO_URL", "FILE_PATH"), help="Analyze a specific file in a repository")

    parser.add_argument("--server-url", default="http://localhost:8000", help="URL of the analysis server")
