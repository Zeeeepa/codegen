...
class CodeIntegrityAnalyzer:
    ...
    def _analyze_functions(self, functions: List[Function]) -> List[Dict[str, Any]]:
        """
        Analyze functions for integrity issues.
        ...
        for func in functions:
            ...
            if self.config["require_docstrings"] and not func.docstring:
                errors.append(
                    {
                        "type": "function_error",
                        ...
                        "severity": self.config["severity_levels"]["missing_docstring"],
                    }
                )

            # Check for empty function
            if not func.body or (isinstance(func.body, list) and not func.body):
                errors.append(
                    {
                        "type": "function_error",
                        ...
                        "severity": self.config["severity_levels"]["empty_function"],
                    }
                )

            # Check for too many parameters
            max_params = cast(int, self.config["max_function_parameters"])
            ...
            max_returns = cast(int, self.config["max_function_returns"])
            if len(func.return_statements) > max_returns:
                errors.append(
                    {
                        "type": "function_error",
                        ...
                        "severity": self.config["severity_levels"]["too_many_returns"],
                    }
                )
                        ...
                        errors.append(
                            {
                                ...
                                "filepath": func.filepath,
                                "line": func.line_range[0],
                                "message": f"Function '{func.name}' has mutable default argument '{param.name}'",
    ...
    def _analyze_classes(self, classes: List[Class]) -> List[Dict[str, Any]]:
        """
        Analyze classes for integrity issues.
        ...
        for cls in classes:
            ...
            if any(re.search(pattern, cls.filepath) for pattern in self.config["ignore_patterns"]):
                continue

            # Check for missing docstring
            if self.config["require_docstrings"] and not cls.docstring:
                errors.append(
                    {
                        "type": "class_error",
                        "error_type": "missing_docstring",
                ...
                errors.append(
                    {
                        ...
                        "filepath": cls.filepath,
                        "line": cls.line_range[0],
                        "message": f"Class '{cls.name}' is empty",
                        "severity": self.config["severity_levels"]["empty_class"],
                    }
                )

            # Check for too many methods
            max_methods = cast(int, self.config["max_class_methods"])
                ...
                errors.append(
                    {
                        ...
                        "severity": self.config["severity_levels"]["too_many_methods"],
                    }
                )

            # Check for too many attributes
            max_attributes = cast(int, self.config["max_class_attributes"])
                ...
                errors.append(
                    {
                        ...
                        "severity": self.config["severity_levels"]["too_many_attributes"],
                    }
                )
                ...
                errors.append(
                    {
                        ...
                        "severity": self.config["severity_levels"]["missing_init"],
                    }
                )
                ...
                errors.append(
                    {
                        ...
                        "name": (
                            unused_import.name if hasattr(unused_import, "name") else "unknown"
                        ),
