...
class CodebaseContext:
    ...
    def _process_diff_files(
        ...
        skip_uncache = incremental and (
            (
                len(files_to_process[SyncType.DELETE])
                + len(files_to_process[SyncType.REPARSE])
            )
            == 0
        )
        if not skip_uncache:
        ...
        if incremental:
            ...
            files_to_process[SyncType.ADD] = [  # type: ignore
                f for f in files_to_process[SyncType.ADD] if f not in add_to_remove
            ]
            files_to_process[SyncType.REPARSE] = [  # type: ignore
                f
                for f in files_to_process[SyncType.REPARSE]
                if f not in reparse_to_remove
            ]
        ...
        task = self.progress.begin(
            "Adding new files", count=len(files_to_process[SyncType.ADD])
        )
