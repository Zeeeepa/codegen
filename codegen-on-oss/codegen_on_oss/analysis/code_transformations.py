...
def convert_call_to_kwargs(
    ...
    try:
        ...
        if function_def is not None:
            ...
            if len(positional_args) > len(param_names):
                logger.warning(
                    ...
                    f"Not enough parameter names ({len(param_names)}) for "
                    f"positional arguments ({len(positional_args)})"
                )
                return False
        ...
        logger.warning(
            f"Cannot convert call to {function_call.name} to kwargs: "
            f"function definition not available"
        )
        return False
            ...
            for function in file.functions:
                for statement in function.code_block.statements:
                    for call in statement.function_calls:
                        try:
                            # Skip if we're only targeting specific functions and this isn't one
