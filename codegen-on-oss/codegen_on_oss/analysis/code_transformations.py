...
def convert_call_to_kwargs(
    function_call: FunctionCall, function_def: Optional[Function] = None
) -> bool:
    """
    ...
    try:
        ...
        logger.error(
            f"Error converting call to {function_call.name} to kwargs: {str(e)}"
        )
        return False
...
def convert_all_calls_to_kwargs(
    ...
    try:
                ...
                for statement in function.code_block.statements:
                    for call in statement.function_calls:
                        try:
                            ...
                            error_msg = f"Error processing call to {call.name} in {file.path}: {str(e)}"
        ...
        logger.info(
            f"Converted {converted_count} function calls to use keyword arguments"
        )
        logger.info(
        ...
        if failed_conversions:
            logger.warning(
                f"Failed to convert {len(failed_conversions)} function calls"
            )
