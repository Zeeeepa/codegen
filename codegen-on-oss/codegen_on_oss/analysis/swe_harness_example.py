...
def analyze_pr(repo_url: str, pr_number: int, github_token: Optional[str] = None, post_comment: bool = False):
    ...
    if post_comment:
        print("\nComment posted to PR:", "Yes" if results.get("comment_posted", False) else "No")


def analyze_commit(repo_url: str, base_commit: str, head_commit: str, github_token: Optional[str] = None):
...
def create_snapshots_example(repo_url: str, base_commit: str, head_commit: str, github_token: Optional[str] = None):
...
def main():
    ...
    pr_parser.add_argument("--comment", action="store_true", help="Post a comment to the PR with the analysis results")
    ...
    snapshot_parser = subparsers.add_parser("snapshot", help="Demonstrate snapshot creation and comparison")
    snapshot_parser.add_argument("--repo", required=True, help="Repository URL or owner/repo string")
