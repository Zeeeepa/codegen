...
class CommitExampleRunner:
    ...
    def _clone_repo(self, temp_dir: str) -> None:
        ...
        """
        subprocess.run(["git", "clone", self.repo_url, temp_dir], check=True, capture_output=True, text=True)
    ...
    def _get_previous_commit(self, repo_dir: str, commit_hash: str) -> str:
        ...
        """
        result = subprocess.run(["git", "-C", repo_dir, "rev-parse", f"{commit_hash}^"], check=True, capture_output=True, text=True)
    ...
    def _get_default_branch(self, repo_dir: str) -> str:
        ...
        """
        result = subprocess.run(["git", "-C", repo_dir, "symbolic-ref", "refs/remotes/origin/HEAD"], check=True, capture_output=True, text=True)
    ...
    def _checkout_commit(self, repo_dir: str, commit_hash: str, target_dir: str) -> None:
        ...
        subprocess.run(["git", "-C", repo_dir, "worktree", "add", "--detach", target_dir, commit_hash], check=True, capture_output=True, text=True)
    ...
    def _checkout_branch(self, repo_dir: str, branch_name: str, target_dir: str) -> None:
        ...
        subprocess.run(["git", "-C", repo_dir, "worktree", "add", target_dir, branch_name], check=True, capture_output=True, text=True)
