...
class CommitAnalysisResult:
    ...
    def get_summary(self) -> str:
        ...
        if self.is_properly_implemented:
            ...
            summary.append(
                "❌ The commit has implementation issues that should be addressed."
            )
        ...
        if self.metrics_diff:
            ...
            if (
                "lines_added" in self.metrics_diff
                and "lines_removed" in self.metrics_diff
            ):
                summary.append(
                    f"- {self.metrics_diff['lines_added']} lines added, {self.metrics_diff['lines_removed']} lines removed"
                )
...
class CommitAnalyzer:
    ...
    def __init__(
        self, snapshot_manager: SnapshotManager, github_token: Optional[str] = None
    ):
        """
    ...
    def analyze_commit(
        ...
        base_snapshot = self.snapshot_manager.create_snapshot_from_commit(
            repo_url, base_commit
        )
        head_snapshot = self.snapshot_manager.create_snapshot_from_commit(
            repo_url, head_commit
        )
    ...
    def _analyze_snapshots(
        self, base_snapshot: str, head_snapshot: str
    ) -> Dict[str, Any]:
        """
        ...
        files_modified = [
            ...
            and head_files[path].content != base_files[path].content
        ]
        ...
        metrics_diff = {
            "complexity_change": head_complexity["overall_complexity"]
            - base_complexity["overall_complexity"],
            "function_count_change": head_complexity["function_count"]
            - base_complexity["function_count"],
            "class_count_change": head_complexity["class_count"]
            - base_complexity["class_count"],
            "lines_added": sum(
                len(head_files[path].content.splitlines()) for path in files_added
            ),
            "lines_removed": sum(
                len(base_files[path].content.splitlines()) for path in files_removed
            ),
        }
        ...
        quality_assessment = {
            ...
            "overall_assessment": self._get_overall_assessment(
                is_properly_implemented, issues
            ),
        }
    ...
    def _check_for_issues(
        ...
        if (
            head_complexity["overall_complexity"]
            > base_complexity["overall_complexity"] + 10
        ):
            issues.append(
                CommitAnalysisIssue(
                    severity="warning",
        ...
        for file_path in files_added:
            if file_path.endswith((".py", ".js", ".ts")) and not file_path.endswith(
                ...
                if not any(
                    test_pattern in files_added for test_pattern in test_file_patterns
                ):
                    issues.append(
                        CommitAnalysisIssue(
                            severity="info",
            ...
            if file_path in head_complexity["files"]:
                ...
                if file and file.content:
                    todo_matches = re.finditer(
                        r"#\s*TODO|//\s*TODO|/\*\s*TODO", file.content
                    )
    ...
    def format_analysis_report(self, analysis_results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"- Function Count Change: {metrics_diff['function_count_change']:+d}"
        )
        ...
        report.append("\n## Complexity Analysis:")
        report.append(
            f"- Base Complexity: {analysis_results['base_complexity']['overall_complexity']}"
        )
        report.append(
            f"- Head Complexity: {analysis_results['head_complexity']['overall_complexity']}"
        )
        ...
        if quality["is_properly_implemented"]:
            report.append(
                "This commit is properly implemented and has no significant issues."
            )
        else:
            report.append(
                "This commit has issues that should be addressed before merging."
            )
...
def analyze_commit_from_repo_and_commit(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        return CommitAnalysisResult(
            is_properly_implemented=results["quality_assessment"][
