...
class ProjectRegistrationRequest(BaseModel):
    ...
    webhook_url: Optional[str] = Field(None, description="Webhook URL to notify when analysis is complete")
...
class WebhookRegistrationRequest(BaseModel):
    ...
    events: list[str] = Field(["pr", "commit", "branch"], description="Events to trigger the webhook")
    ...
    for temp_dir in temp_dirs:
        if os.path.exists(temp_dir):
            try:
                ...
                logger.exception(f"Error cleaning up temporary directory {temp_dir}: {e}")
...
async def root():
    ...
    return {
        ...
        "endpoints": [
            ...
            {"path": "/register_project", "method": "POST", "description": "Register a project for analysis"},
            {"path": "/register_webhook", "method": "POST", "description": "Register a webhook"},
            {"path": "/analyze_function", "method": "POST", "description": "Analyze a specific function"},
            {"path": "/analyze_feature", "method": "POST", "description": "Analyze a specific feature"},
        ],
    }
...
async def analyze_repo(request: RepoAnalysisRequest):
    ...
    try:
        ...
        result = {"repo_url": request.repo_url, "status": "success", "summary": analyzer.get_codebase_summary(), "complexity": analyzer.analyze_complexity(), "imports": analyzer.analyze_imports()}
        ...
        logger.exception(f"Error analyzing repository {request.repo_url}: {e}")
...
async def analyze_commit(request: CommitAnalysisRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        result = CodeAnalyzer.analyze_commit_from_repo_and_commit(repo_url=request.repo_url, commit_hash=request.commit_hash)
        ...
        logger.exception(f"Error analyzing commit {request.commit_hash} in repository {request.repo_url}: {e}")
        # Schedule cleanup of temporary directories
        background_tasks.add_task(cleanup_temp_dirs, temp_dirs)
        return {"repo_url": request.repo_url, "commit_hash": request.commit_hash, "status": "error", "error": str(e)}
...
async def compare_branches(request: BranchComparisonRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.exception(f"Error comparing branches {request.base_branch} and {request.compare_branch} in repository {request.repo_url}: {e}")
        # Schedule cleanup of temporary directories
        background_tasks.add_task(cleanup_temp_dirs, temp_dirs)
        return {"repo_url": request.repo_url, "base_branch": request.base_branch, "compare_branch": request.compare_branch, "status": "error", "error": str(e)}
...
async def analyze_pr(request: PullRequestAnalysisRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.exception(f"Error analyzing PR #{request.pr_number} in repository {request.repo_url}: {e}")
        # Schedule cleanup of temporary directories
        background_tasks.add_task(cleanup_temp_dirs, temp_dirs)
        return {"repo_url": request.repo_url, "pr_number": request.pr_number, "status": "error", "error": str(e)}
...
async def register_project(request: ProjectRegistrationRequest):
    ...
    try:
        ...
        logger.exception(f"Error registering project {request.name} for analysis: {e}")
...
async def register_webhook(request: WebhookRegistrationRequest):
    ...
    try:
        ...
        webhook_response = WebhookResponse(webhook_id="1", project_id=request.project_id, webhook_url=request.webhook_url, events=request.events, created_at=datetime.now())

        return webhook_response
    except Exception as e:
        logger.exception(f"Error registering webhook for project {request.project_id}: {e}")
        return {"status": "error", "error": str(e)}
...
async def analyze_function(request: FunctionAnalysisRequest):
    ...
    try:
        ...
        logger.exception(f"Error analyzing function {request.function_name} in repository {request.repo_url}: {e}")
        return {"status": "error", "error": str(e)}
...
async def analyze_feature(request: FeatureAnalysisRequest):
    ...
    try:
        ...
        logger.exception(f"Error analyzing feature {request.feature_path} in repository {request.repo_url}: {e}")
        return {"status": "error", "error": str(e)}
