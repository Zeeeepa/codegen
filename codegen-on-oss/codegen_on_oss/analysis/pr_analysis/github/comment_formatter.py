    ...
    def format_summary_comment(self, results: Dict[str, Any]) -> str:
        ...
        for rule_id, rule_result in results.items():
            ...
            for i, issue in enumerate(rule_issues[:5]):
                ...
                location = f"{file_path}:{line_number}" if file_path and line_number else file_path if file_path else "N/A"
                
                comment += f"{severity_icon} **{location}**: {issue.get('message', 'No message')}\n\n"
