...
class CodeStyleRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            return self.warning(
                f"Found {len(issues)} code style issues",
                {'issues': issues}
            )
        else:
            return self.error(
...
class TestCoverageRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        test_patterns = self.get_config('test_patterns', [r'.*_test\.py$', r'.*\.test\.(js|ts|tsx|jsx)$', r'test_.*\.py$'])
...
class SecurityVulnerabilityRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        Returns:
            Rule results
        """
        logger.info(f"Running security vulnerability rule for PR #{self.context.pull_request.number}")
    ...
    def _check_file_security(self, file: File) -> List[Dict[str, Any]]:
        ...
        secret_patterns = self.get_config('secret_patterns', [
            r'password\s*=\s*[\'"][^\'"]+[\'"]',
            r'secret\s*=\s*[\'"][^\'"]+[\'"]',
            r'api_key\s*=\s*[\'"][^\'"]+[\'"]',
            r'token\s*=\s*[\'"][^\'"]+[\'"]'
