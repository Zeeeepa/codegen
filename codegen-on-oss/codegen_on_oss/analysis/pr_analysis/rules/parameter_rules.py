...
class ParameterTypeRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            return self.warning(f"Found {len(issues)} parameter type issues", {"issues": issues})
        else:
            return self.error(f"Found {len(issues)} parameter type issues", {"issues": issues})
...
class ParameterValidationRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            return self.warning(
                f"Found {len(issues)} parameter validation issues", {"issues": issues}
            )
        else:
            return self.error(
                f"Found {len(issues)} parameter validation issues", {"issues": issues}
                ...
                for pattern in func_def_patterns:
                    ...
                    if match:
                        ...
                        for param in params:
                            ...
                            validated = any(
                                re.search(pattern, body) for pattern in validation_patterns
                            )
                            if not validated:
                                issues.append(
                                    {
