...
class PerformanceRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            return self.warning(
                f"Found {len(issues)} potential performance issues", {"issues": issues}
            )
        else:
            return self.error(
                f"Found {len(issues)} potential performance issues", {"issues": issues}
        ...
        if file.filename.endswith(".py"):
            ...
            inefficient_patterns = [
                (
                    ...
                    "Using range(len(x)) instead of iterating directly",
                ),
                (
                    r"\[.*for\s+.*\s+in\s+.*\s+if\s+.*\]",
                    "List comprehension with filter might be better as a generator expression",
                ),
                (
                    r"\.append\(.*\)\s+.*\.append\(.*\)",
                    "Multiple .append() calls might be better as extend() or a list comprehension",
                ),
            ]
            ...
            for pattern, message in inefficient_patterns:
                ...
                for match in matches:
                    line_number = content[: match.start()].count("\n") + 1
                    issues.append({"file": file.filename, "line": line_number, "message": message})
            ...
            inefficient_patterns = [
                (
                    r"\.forEach\(.*\).*\.filter\(.*\)",
                    "Chaining .forEach() and .filter() might be inefficient",
                ),
                (r"\.indexOf\(.*\)\s*!=\s*-1", "Using .indexOf() != -1 instead of .includes()"),
                (
                    r"for\s*\(.*\s+of\s+.*\).*\.push\(",
                    "Using for...of with .push() might be better as .map() or .flatMap()",
                ),
            ]
...
class ErrorHandlingRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            return self.warning(f"Found {len(issues)} error handling issues", {"issues": issues})
        else:
            return self.error(f"Found {len(issues)} error handling issues", {"issues": issues})
...
class DocumentationRule(BaseRule):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if not issues:
            ...
            return self.warning(f"Found {len(issues)} documentation issues", {"issues": issues})
        else:
            return self.error(f"Found {len(issues)} documentation issues", {"issues": issues})
    ...
    def _check_file_documentation(self, file: File) -> List[Dict[str, Any]]:
        ...
        if file.filename.endswith(".py"):
            ...
            if not re.search(r'^""".*"""', content, re.DOTALL) and not re.search(
                ...
                issues.append(
                    {"file": file.filename, "line": 1, "message": "Module is missing a docstring"}
                )
            ...
            for match in class_matches:
                ...
                if not re.search(r'^\s*"""', next_lines, re.MULTILINE) and not re.search(
                    r"^\s*\'\'\'", next_lines, re.MULTILINE
                ):
                    issues.append(
                        {
                            "file": file.filename,
            ...
            if not re.search(r"^/\*\*.*\*/", content, re.DOTALL):
                issues.append(
                    {"file": file.filename, "line": 1, "message": "File is missing a JSDoc comment"}
                )
            ...
            for match in class_matches:
                ...
                ]  # Look at the previous 200 characters
