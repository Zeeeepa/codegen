...
class PRAnalyzer:
    ...
    def initialize(self, repo_url: str, pr_number: int) -> None:
        ...
        self.context = AnalysisContext(
            repository=repository, pull_request=pull_request, config=self.config
        )
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Analyzing PR #{self.context.pull_request.number} in {self.context.repository.full_name}"
        )
        ...
        return {"rule_results": rule_results, "report": report}
    ...
    def post_results(self, results: Dict[str, Any]) -> None:
        ...
        self.github_client.post_pr_comment(
            self.context.repository, self.context.pull_request, report_markdown
        )
