...
class RuleEngine:
    ...
    def create_rule_instance(
        self, rule_id: str, config: Optional[Dict[str, Any]] = None
    ) -> BaseRule:
        """
    ...
    def run_rules(self, context: AnalysisContext) -> Dict[str, Any]:
        ...
        for rule in self.rules:
            try:
                ...
                logger.info(
                    f"Rule {rule.rule_id} completed with {len(rule_result.get('issues', []))} issues"
                )
            except Exception as e:
                logger.exception(f"Error running rule {rule.rule_id}: {e}")
                results[rule.rule_id] = {"error": str(e), "issues": []}
