...
def create_chart(title: str, chart_type: str, labels: List[str], values: List[Union[int, float]]) -> Dict[str, Any]:
    """
...
def create_graph(title: str, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
...
def render_table_as_ascii(table: Dict[str, Any]) -> str:
    ...
    for row in rows:
        data_row = '| ' + ' | '.join(str(cell).ljust(col_widths[i]) if i < len(col_widths) else str(cell) for i, cell in enumerate(row)) + ' |'
        result += data_row + '\n'
