...
class DiffAnalyzer:
    ...
    def assess_risk(self) -> Dict[str, Any]:
        ...
        num_removed_functions = sum(
            1 for change in function_changes.values() if change == "removed"
        )
        num_modified_functions = sum(
            1 for change in function_changes.values() if change == "modified"
        )

        total_complexity_change = sum(complexity_changes.values())
        avg_complexity_change = (
            total_complexity_change / len(complexity_changes) if complexity_changes else 0
        )
    ...
    def format_summary_text(self) -> str:
        ...
        summary.append(
            f"Files: {num_added_files} added, {num_removed_files} removed, "
            f"{num_modified_files} modified"
        )
        ...
        summary.append(
            f"Functions: {num_added_functions} added, {num_removed_functions} removed, "
            f"{num_modified_functions} modified"
        )
        ...
        summary.append(
            f"Complexity: {total_complexity_change:.2f} total change, "
            f"{avg_complexity_change:.2f} average change"
        )

        # Risk assessment
        summary.append(
            f"Risk: {risk_assessment['overall_risk']} overall, "
            f"{risk_assessment['file_risk']} file, "
            f"{risk_assessment['function_risk']} function, "
            f"{risk_assessment['complexity_risk']} complexity"
        )
    ...
    def get_detailed_metrics(self) -> Dict[str, Any]:
        ...
        detailed_metrics = {
            ...
            "complexity_changes": {
                ...
                "average": sum(complexity_changes.values()) / len(complexity_changes)
    ...
    def _calculate_change_impact(self) -> Dict[str, Any]:
        ...
        self.head_snapshot.get_dependencies()
        ...
        max_impacted_functions = max(
            (len(deps) for deps in changed_dependencies.values()), default=0
        )
