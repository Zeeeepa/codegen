...
class DiffAnalyzer:
    ...
    def analyze_import_changes(self) -> dict[str, dict[str, list[str]]]:
        ...
        all_files = set(self.original.import_metrics.keys()).union(set(self.modified.import_metrics.keys()))
        ...
        for filepath in all_files:
            ...
            self._import_diffs[filepath] = {"added": list(added_imports), "deleted": list(deleted_imports), "unchanged": list(unchanged_imports)}
    ...
    def analyze_complexity_changes(self) -> dict[str, dict[str, Any]]:
        ...
        common_functions = [func_name for func_name, change_type in function_diffs.items() if change_type in ["modified", "unchanged", "moved"]]
        ...
        for func_name in common_functions:
            ...
            self._complexity_changes[func_name] = {"original": original_complexity, "modified": modified_complexity, "delta": delta, "percent_change": percent_change}
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        if complexity_stats["total"] > 0:
            complexity_stats["avg_delta"] = sum(change["delta"] for change in complexity_changes.values()) / complexity_stats["total"]
            complexity_stats["max_increase"] = max((change["delta"] for change in complexity_changes.values()), default=0)
            complexity_stats["max_decrease"] = min((change["delta"] for change in complexity_changes.values()), default=0)
        else:
    ...
    def get_detailed_file_diff(self, filepath: str) -> Optional[list[str]]:
        ...
        diff = list(difflib.unified_diff(original_lines, modified_lines, fromfile=f"a/{filepath}", tofile=f"b/{filepath}", lineterm=""))
    ...
    def get_high_risk_changes(self) -> dict[str, list[dict[str, Any]]]:
        ...
        """
        high_risk = {"complexity_increases": [], "core_file_changes": [], "interface_changes": [], "dependency_changes": []}
        ...
        for func_name, change in complexity_changes.items():
            ...
            if change["percent_change"] > 30 or change["delta"] > 5:
                high_risk["complexity_increases"].append(
                    {"function": func_name, "original": change["original"], "modified": change["modified"], "delta": change["delta"], "percent_change": change["percent_change"]}
                )
        ...
        for filepath, change_type in file_changes.items():
            if change_type in ["modified", "deleted"] and filepath in self.original.file_metrics:
                ...
                if self.original.file_metrics[filepath]["symbol_count"] > 10:
                    high_risk["core_file_changes"].append({"filepath": filepath, "change_type": change_type, "symbol_count": self.original.file_metrics[filepath]["symbol_count"]})
        ...
        for func_name, change_type in function_changes.items():
            if change_type == "modified" and func_name in self.original.function_metrics:
                ...
                if original_params != modified_params:
                    high_risk["interface_changes"].append({"function": func_name, "original_params": original_params, "modified_params": modified_params})
        ...
        for filepath, changes in import_changes.items():
            if changes["added"] or changes["deleted"]:
                high_risk["dependency_changes"].append({"filepath": filepath, "added_imports": changes["added"], "deleted_imports": changes["deleted"]})

        return high_risk

    def format_summary_text(self) -> str:
        """Format the diff summary as a human-readable text.

        Returns:
            A formatted string with the diff summary
        """
