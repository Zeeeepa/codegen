...
class SWEHarnessAgent:
    ...
    def _initialize_agent(self):
        ...
        if self.use_agent and not self.agent_api_key:
            ...
            logger.warning(
                "Set CODEGEN_API_KEY environment variable or pass agent_api_key"
            )
        ...
        if self.use_agent and self.agent_api_key:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Failed to connect to agent API: {response.status_code}"
                    )
                    self.use_agent = False
    ...
    def _get_agent_analysis(
        ...
        if not self.agent or not self.agent_api_key:
            logger.info(
                "Agent-based commit analysis requested but agent is not initialized"
            )
            return {
    ...
    def _get_agent_pr_analysis(
        ...
        if not self.agent or not self.agent_api_key:
            logger.info(
                "Agent-based PR analysis requested but agent is not initialized"
            )
            return {
                "status": "not_initialized",
                "message": "Agent-based analysis is not initialized",
        ...
        try:
            ...
            for file in files:
                try:
                    ...
                    logger.warning(
                        f"Unicode decode error for {file.filename}: {str(e)}"
                    )
                except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--agent-api-key", help="API key for the LLM-based agent service"
    )
