...
class EventBus:
    ...
    def subscribe_async(self, event_type: EventType, handler: AsyncEventHandler) -> None:
        ...
        logger.debug(f"Subscribed async handler {handler.__name__} to {event_type.name}")
    ...
    def unsubscribe_async(self, event_type: EventType, handler: AsyncEventHandler) -> None:
        ...
        if handler in self.async_subscribers[event_type]:
            ...
            logger.debug(f"Unsubscribed async handler {handler.__name__} from {event_type.name}")
    ...
    async def _call_async_handler(self, handler: AsyncEventHandler, event: Event) -> None:
