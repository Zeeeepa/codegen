...
class Snapshot(Base):
    ...
    entities = relationship(
        "CodeEntity", secondary=snapshot_entity_association, back_populates="snapshots"
    )
    analyses = relationship(
        "Analysis", back_populates="snapshot", cascade="all, delete-orphan"
    )
...
class CodeEntity(Base):
    ...
    # Relationships
    snapshots = relationship(
        "Snapshot", secondary=snapshot_entity_association, back_populates="entities"
    )
    analyses = relationship(
        "Analysis", secondary=analysis_entity_association, back_populates="entities"
    )
...
class FunctionEntity(CodeEntity):
    ...
    parent_class_id = Column(
        UUID(as_uuid=True), ForeignKey("class_entities.id"), nullable=True
    )
...
class Analysis(Base):
    ...
    analysis_type = Column(
        String(50), nullable=False
    )  # 'dependency', 'complexity', 'security', etc.
    timestamp = Column(DateTime, default=datetime.utcnow)
    ...
    entities = relationship(
        "CodeEntity", secondary=analysis_entity_association, back_populates="analyses"
    )
