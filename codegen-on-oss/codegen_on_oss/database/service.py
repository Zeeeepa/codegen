...
class DatabaseService:
    ...
    def get_or_create_repository(
        self, url: str, name: str, description: Optional[str] = None
    ) -> Repository:
    ...
    def get_snapshot_by_commit(
        self, repository_id: Any, commit_sha: str
    ) -> Optional[Snapshot]:
    ...
    def store_snapshot(
        self, codebase_snapshot: CodebaseSnapshot, repository_id: Any
    ) -> Snapshot:
        ...
        with self.get_session() as session:
            ...
            for (
                qualified_name,
                func_metric,
            ) in codebase_snapshot.function_metrics.items():
    ...
    def compare_snapshots(
        self, snapshot_id_1: Any, snapshot_id_2: Any
    ) -> Dict[str, Any]:
        """Compare two snapshots and return the differences."""
        with self.get_session() as session:
            snapshot1 = (
                session.query(Snapshot).filter(Snapshot.id == snapshot_id_1).first()
            )
            snapshot2 = (
                session.query(Snapshot).filter(Snapshot.id == snapshot_id_2).first()
            )
            ...
            metrics = {
                ...
                "unchanged_count": len(
                    [
                        name
                        for name in entities1
                        if name in entities2 and name not in modified
                    ]
                ),
            }
