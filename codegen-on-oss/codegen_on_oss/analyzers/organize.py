    ...
    if not os.path.exists(init_file):
        with open(init_file, "w") as f:
            ...
            if dir_name == "issues":
                ...
                f.write(")\n\n")
                f.write(
                    "from codegen_on_oss.analyzers.issues.issue_analyzer import IssueAnalyzer\n"
                )

            # Add imports for common types if this is the dependencies directory
            if dir_name == "dependencies":
                f.write(
                    "from codegen_on_oss.analyzers.dependencies.dependencies import (\n"
                )
...
def organize_analyzers(base_path=None):  # noqa: C901
    """
    ...
    # Create directories
    create_directory(base_path, "issues")
    create_directory(base_path, "dependencies")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--organize", action="store_true", help="Reorganize existing files"
    )
    parser.add_argument(
        "--extend", action="store_true", help="Extend structure with new directories"
    )
