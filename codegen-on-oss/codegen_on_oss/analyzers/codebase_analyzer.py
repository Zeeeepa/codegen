...
class CodebaseAnalyzer:
    ...
    def analyze(
        self, analysis_type: AnalysisType = AnalysisType.CODEBASE
    ) -> dict[str, Any]:
        """
        ...
        if analysis_type == AnalysisType.CODEBASE:
            ...
            result["static_analysis"] = self._perform_static_analysis(
                self.base_codebase
            )
    ...
    def _perform_static_analysis(self, codebase: Codebase) -> dict[str, Any]:
        ...
        if context:
            ...
            analysis_result["dependency_chains"] = self._analyze_dependency_chains(
                context
            )

            # Analyze circular dependencies
            analysis_result["circular_dependencies"] = self._find_circular_dependencies(
                context
            )
    ...
    def _analyze_dependency_chains(self, context: CodebaseContext) -> dict[str, Any]:
        ...
        result = {"long_chains": [], "critical_paths": []}
            ...
            try:
                ...
                if paths and len(paths[0]) > 3:
                    path_info = {
                        ...
                        "targets": [
                            paths[0][-1].name
                            if hasattr(paths[0][-1], "name")
                            else str(paths[0][-1])
                        ],
                        "length": len(paths[0]),
                        "path": [
                            n.name if hasattr(n, "name") else str(n) for n in paths[0]
                        ],
                    }
    ...
    def _find_circular_dependencies(self, context: CodebaseContext) -> dict[str, Any]:
        ...
        result = {"circular_imports": [], "circular_function_calls": []}
                ...
                if all(
                    ...
                    if all(isinstance(node, SourceFile) for node in cycle):
                        result["circular_imports"].append({
                            "files": [
                                node.path if hasattr(node, "path") else str(node)
                                for node in cycle
                            ],
                            "length": len(cycle),
                        })
                    ...
                    # Check if all nodes in the cycle are functions
                        ...
                        if (
                            len(cycle) > 0
                            and hasattr(cycle[0], "file")
                            and hasattr(cycle[0].file, "file_path")
                        ):
                            self.issues.append(
                                Issue(
                                    file=cycle[0].file.file_path,
                                    line=cycle[0].line
                                    if hasattr(cycle[0], "line")
                                    else None,
                                    message="Circular function call dependency detected",
                                    severity=IssueSeverity.ERROR,
                                    symbol=cycle[0].name
                                    if hasattr(cycle[0], "name")
                                    else str(cycle[0]),
    ...
    def _analyze_module_coupling(self, context: CodebaseContext) -> dict[str, Any]:
        ...
        result = {"high_coupling": [], "low_cohesion": []}
        ...
        for file_node in [
            node for node in context.nodes if isinstance(node, SourceFile)
        ]:
            ...
            file_symbols = [
                ...
                and hasattr(node, "file")
                and hasattr(node.file, "path")
                and str(node.file.path) == file_path
            ]
        ...
        result["high_coupling"] = [
            ...
            > 5  # Only include if it has more than 5 dependencies
        ]
    ...
    def _analyze_call_hierarchy(self, context: CodebaseContext) -> dict[str, Any]:
        ...
        result = {"entry_points": [], "leaf_functions": [], "deep_call_chains": []}
        ...
        for node in context.nodes:
            if isinstance(node, Function) and hasattr(node, "name"):
                ...
                has_callers = False
                for _pred, _, data in context.in_edges(node, data=True):
        ...
        for node in context.nodes:
            if isinstance(node, Function) and hasattr(node, "name"):
                ...
                has_callees = False
                for _, _succ, data in context.out_edges(node, data=True):
                    if "type" in data and data["type"] == EdgeType.CALLS:
                        has_callees = True
                        break
            ...
            try:
                ...
                if len(longest_path) > 3:  # Only record if path length > 3
                    call_chain = {
                        ...
                        "calls": [
                            func.name for func in longest_path if hasattr(func, "name")
                        ],
                    }
    ...
    def _compare_codebases(self) -> dict[str, Any]:
        ...
        if not self.base_context or not self.pr_context:
            raise ValueError(
                "Both base and PR CodebaseContext objects must be initialized"
            )
        ...
        for base_node in base_nodes:
            if hasattr(base_node, "name"):
                ...
                pr_node = next(
                    (n for n in pr_nodes if hasattr(n, "name") and n.name == node_name),
                    None,
                )
        ...
        for pr_node in pr_nodes:
            if hasattr(pr_node, "name"):
                ...
                if not any(
                    hasattr(n, "name") and n.name == node_name for n in base_nodes
                ):
        ...
        if self.modified_symbols:
            ...
            for symbol_name in self.modified_symbols:
                ...
                if (
                    base_symbol
                    and hasattr(base_symbol, "symbol_type")
                    and base_symbol.symbol_type == SymbolType.Function
                    and pr_symbol
                    and hasattr(pr_symbol, "symbol_type")
                    and pr_symbol.symbol_type == SymbolType.Function
                ):
                    ...
                    for dep in base_dependencies:
                        if hasattr(dep, "name") and not any(
                            hasattr(d, "name") and d.name == dep.name
                            for d in pr_dependencies
                        ):
                    ...
                    for base_param in base_params:
                        if not any(
                            pr_param.name == base_param.name
                            for pr_param in pr_params
                            if hasattr(pr_param, "name")
                        ):
                    ...
                    for base_param in base_params:
                        for pr_param in pr_params:
                            if (
                                hasattr(base_param, "name")
                                and hasattr(pr_param, "name")
                                and base_param.name == pr_param.name
                            ):
                                base_type = (
                                    str(base_param.type)
                                    if hasattr(base_param, "type") and base_param.type
                                    else None
                                )
                                pr_type = (
                                    str(pr_param.type)
                                    if hasattr(pr_param, "type") and pr_param.type
                                    else None
                                )
                    ...
                    base_return_type = (
                        str(base_func.return_type)
                        if hasattr(base_func, "return_type") and base_func.return_type
                        else None
                    )
                    pr_return_type = (
                        str(pr_func.return_type)
                        if hasattr(pr_func, "return_type") and pr_func.return_type
                        else None
                    )
                    ...
                    if hasattr(base_func, "call_sites") and hasattr(
                        pr_func, "call_sites"
                    ):
                        ...
                        for call_site in base_call_sites:
                            if len(removed_params) > 0 and not all(
                                param.has_default
                                for param in base_params
                                if hasattr(param, "name")
                                and param.name in removed_params
                            ):
        ...
        base_stats = {
            ...
            "functions": len(list(self.base_codebase.functions))
            if hasattr(self.base_codebase, "functions")
            else 0,
            "classes": len(list(self.base_codebase.classes))
            if hasattr(self.base_codebase, "classes")
            else 0,
            "imports": len(list(self.base_codebase.imports))
            if hasattr(self.base_codebase, "imports")
            else 0,
        }

        pr_stats = {
            "files": len(list(self.pr_codebase.files)),
            "functions": len(list(self.pr_codebase.functions))
            if hasattr(self.pr_codebase, "functions")
            else 0,
            "classes": len(list(self.pr_codebase.classes))
            if hasattr(self.pr_codebase, "classes")
            else 0,
            "imports": len(list(self.pr_codebase.imports))
            if hasattr(self.pr_codebase, "imports")
            else 0,
        }
    ...
    def _find_dead_code(self, codebase: Codebase) -> dict[str, Any]:
        ...
        if hasattr(codebase, "functions"):
            for func in codebase.functions:
                ...
                if len(func.call_sites) == 0:
                    ...
                    if (hasattr(func, "is_magic") and func.is_magic) or (
                        hasattr(func, "name") and func.name in ["main", "__main__"]
                    ):
                        continue
        ...
        if hasattr(codebase, "global_vars"):
            for var in codebase.global_vars:
                ...
                if len(var.symbol_usages) == 0:
                    ...
                    file_path = (
                        var.file.file_path
                        if hasattr(var, "file") and hasattr(var.file, "file_path")
                        else "unknown"
                    )
        ...
        dead_code["counts"] = {
            ...
            "total": len(dead_code["unused_functions"])
    ...
    def _check_function_parameters(self, codebase: Codebase) -> dict[str, Any]:
        ...
        for func in codebase.functions:
            ...
            if hasattr(codebase, "functions"):
                ...
                overloads = [
                    ...
                    if hasattr(f, "name") and f.name == func_name and f != func
                ]
                    ...
                    for overload in overloads:
                        ...
                        for param in func.parameters:
                            ...
                            matching_params = [
                                ...
                                if hasattr(p, "name") and p.name == param.name
                            ]

                            for matching_param in matching_params:
                                if (
                                    hasattr(matching_param, "type")
                                    and matching_param.type
                                    and str(matching_param.type) != str(param.type)
                                ):
        ...
        parameter_issues["counts"] = {
            ...
            "total": len(parameter_issues["missing_types"])
    ...
    def _check_error_handling(self, codebase: Codebase) -> dict[str, Any]:
        ...
        for func in codebase.functions:
            ...
            if hasattr(func, "symbol_type") and func.symbol_type == SymbolType.Class:
                ...
                if hasattr(func, "name") and (
                    "Error" in func.name or "Exception" in func.name
                ):
                    cls = cast(Class, func)
                    ...
                    if hasattr(cls, "superclasses"):
                        superclass_names = [
                            sc.name for sc in cls.superclasses if hasattr(sc, "name")
                        ]
                        if any(
                    ...
                    if (
                        is_exception
                        and hasattr(cls, "symbol_usages")
                        and not any(
                            "raise" in str(usage) for usage in cls.symbol_usages
                        )
                    ):
        ...
        error_handling["counts"] = {
            ...
            "total": len(error_handling["bare_excepts"])
            + len(error_handling["pass_in_except"])
    ...
    def _check_call_sites(self, codebase: Codebase) -> dict[str, Any]:
        ...
        call_site_issues = {"wrong_parameter_count": [], "wrong_return_type_usage": []}
        ...
        call_site_issues["counts"] = {
            ...
            "total": len(call_site_issues["wrong_parameter_count"])
            + len(call_site_issues["wrong_return_type_usage"]),
        }
    ...
    def _check_imports(self, codebase: Codebase) -> dict[str, Any]:
        """Check for import issues."""
        import_issues = {"circular_imports": [], "wildcard_imports": []}
        ...
        try:
                ...
                for imp in file.imports:
                    if hasattr(imp, "imported_symbol") and imp.imported_symbol:
                        ...
                        if hasattr(imported_symbol, "file") and imported_symbol.file:
                            imported_file_path = (
                                imported_symbol.file.file_path
                                if hasattr(imported_symbol.file, "file_path")
                                else str(imported_symbol.file)
                            )
            ...
            for file_path, imports in dependency_map.items():
                ...
                for imp in imports:
                    if (
                        imp in dependency_map
                    ):  # Only add edges for files that exist in our dependency map
            ...
            logger.exception(f"Error detecting circular imports: {e}")
        ...
        import_issues["counts"] = {
            ...
            "total": len(import_issues["circular_imports"])
            + len(import_issues["wildcard_imports"]),
        }
    ...
    def _check_inheritance(self, codebase: Codebase) -> dict[str, Any]:
        ...
        inheritance_issues["counts"] = {
            ...
            "inconsistent_interfaces": len(
                inheritance_issues["inconsistent_interfaces"]
            ),
            "total": len(inheritance_issues["deep_inheritance"])
            + len(inheritance_issues["multiple_inheritance"])
    ...
    def _analyze_code_complexity(self, codebase: Codebase) -> dict[str, Any]:
        ...
        complexity["counts"] = {
            ...
            "total": len(complexity["complex_functions"])
            + len(complexity["long_functions"])
    ...
    def _check_file_issues(self, file: SourceFile) -> None:
        ...
        if hasattr(file, "symbols"):
            ...
            toplevel_classes = [
                ...
                if hasattr(s, "symbol_type") and s.symbol_type == SymbolType.Class
            ]
            toplevel_functions = [
                s
                for s in file.symbols
                if hasattr(s, "symbol_type") and s.symbol_type == SymbolType.Function
            ]
            ...
            if len(toplevel_classes) > 0 and len(toplevel_functions) > 5:
                self.issues.append(
                    Issue(
                        file=file_path,
                        line=None,
                        message="File mixes classes and many functions at the top level",
                        severity=IssueSeverity.INFO,
    ...
    def _check_function_for_issues(self, func: Function) -> None:
        ...
        if hasattr(func, "parameters"):
            missing_types = [
                p.name
                for p in func.parameters
                if hasattr(p, "name") and (not hasattr(p, "type") or not p.type)
            ]
            if missing_types:
                self.issues.append(
                    Issue(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Codebase and PR Analyzer"
    )
    ...
    parser.add_argument(
        "--analysis-type",
        choices=["codebase", "pr", "comparison"],
        default="codebase",
