...
class CodeQualityAnalyzer(BaseCodeAnalyzer):
    ...
    def analyze(
        self, analysis_type: AnalysisType = AnalysisType.CODE_QUALITY
    ) -> dict[str, Any]:
        """
    ...
    def _find_dead_code(self) -> dict[str, Any]:
        ...
        if hasattr(self.base_codebase, "functions"):
            for func in self.base_codebase.functions:
                ...
                if (
                    hasattr(func, "file")
                    and hasattr(func.file, "filepath")
                    and "test" in func.file.filepath
                ):
                    continue
        ...
        if hasattr(self.base_codebase, "classes"):
            for cls in self.base_codebase.classes:
                ...
                if (
                    hasattr(cls, "file")
                    and hasattr(cls.file, "filepath")
                    and "test" in cls.file.filepath
                ):
                    continue
        ...
        if hasattr(self.base_codebase, "functions"):
            for func in self.base_codebase.functions:
                if not hasattr(func, "code_block") or not hasattr(
    ...
    def _calculate_maintainability(self) -> dict[str, Any]:
        ...
        if hasattr(self.base_codebase, "functions"):
            for func in self.base_codebase.functions:
                ...
                # Count lines of code
                loc = (
                    len(func.code_block.source.split("\n"))
                    if hasattr(func, "code_block")
                    and hasattr(func.code_block, "source")
                    else 0
                )
