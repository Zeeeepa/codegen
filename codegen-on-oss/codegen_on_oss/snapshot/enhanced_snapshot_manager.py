...
from codegen_on_oss.database.repositories import RepositoryRepository, SnapshotRepository, FileRepository
...
class EnhancedSnapshotManager:
    ...
    def __init__(self, db_session: Session, storage_path: Optional[str] = None):
        ...
        self.storage_path = storage_path or os.environ.get("CODEGEN_SNAPSHOT_STORAGE", "./snapshots")
    ...
    def _process_files(
        ...
        for file_path in all_files:
            ...
            if parent_snapshot and file_path in parent_files and parent_files[file_path].content_hash == content_hash:
    ...
    def compare_snapshots(self, snapshot_id_1: str, snapshot_id_2: str) -> Dict[str, Any]:
        ...
        files_1 = {file.filepath: file for file in self.file_repository.get_files_for_snapshot(snapshot_1.id)}
        files_2 = {file.filepath: file for file in self.file_repository.get_files_for_snapshot(snapshot_2.id)}
        
        # Compare files
        added_files = [path for path in files_2 if path not in files_1]
        removed_files = [path for path in files_1 if path not in files_2]
        modified_files = [
            path for path in files_1 if path in files_2 and files_1[path].content_hash != files_2[path].content_hash
        ]
