...
class CodebaseSnapshot:
    ...
    def __init__(self, codebase: Codebase, commit_sha: Optional[str] = None, snapshot_id: Optional[str] = None, timestamp: Optional[datetime] = None):
    ...
    def _capture_import_metrics(self) -> dict[str, list[str]]:
        ...
        for file in self.codebase.files:
            import_metrics[file.filepath] = [
                imp.imported_symbol.qualified_name if hasattr(imp.imported_symbol, "qualified_name") else str(imp.imported_symbol) for imp in file.imports if hasattr(imp, "imported_symbol")
            ]
    ...
    @classmethod
    def deserialize(cls, data: dict[str, Any], codebase: Optional[Codebase] = None) -> "CodebaseSnapshot":
        ...
        snapshot = cls(codebase=codebase, commit_sha=data["metadata"]["commit_sha"], snapshot_id=data["metadata"]["snapshot_id"], timestamp=datetime.fromisoformat(data["metadata"]["timestamp"]))
    ...
    @classmethod
    def load_from_file(cls, filepath: str, codebase: Optional[Codebase] = None) -> "CodebaseSnapshot":
...
class SnapshotManager:
    ...
    def create_snapshot(self, codebase: Codebase, commit_sha: Optional[str] = None, snapshot_id: Optional[str] = None) -> CodebaseSnapshot:
    ...
    def create_codebase_from_repo(self, repo_url: str, commit_sha: Optional[str] = None, github_token: Optional[str] = None) -> Codebase:
    ...
    def snapshot_repo(self, repo_url: str, commit_sha: Optional[str] = None, snapshot_id: Optional[str] = None, github_token: Optional[str] = None) -> CodebaseSnapshot:
