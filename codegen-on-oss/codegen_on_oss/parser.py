...
class CodegenParser:
    ...
    def parse(self, url: str, language: str | None = None, commit_hash: str | None = None):
        ...
        with self.metrics_profiler.start_profiler(name=repo_name, revision=commit_hash, language=language, logger=repo_logger) as profile:
            ...
            class ProfiledCodebase(Codebase):
                def __init__(self, *args, projects: "list[ProjectConfig]", **kwargs):
                    ...
                    profile.revision = (
                        profile.revision or projects[0].repo_operator.head_commit  # assume projects is not empty
                    )
            ...
            ProfiledCodebase.from_repo(repo_name, tmp_dir=str(self.repo_dir.absolute()), commit=commit_hash)
