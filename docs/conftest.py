...
SAMPLE_INPUT_PATH = {
    language: Path(__file__).parent / "samples" / name
    for language, name in SAMPLE_FILENAME.items()
}
SAMPLE_INPUT = {
    language: path.read_text() for language, path in SAMPLE_INPUT_PATH.items()
}
...
def codebase(tmpdir):
    with get_codebase_session(
        ...
        files={SAMPLE_FILENAME[DEFAULT_LANGUAGE]: SAMPLE_INPUT[DEFAULT_LANGUAGE]},
    ) as codebase:
        yield codebase
