...
CATEGORIES = {
    "analyzers": [r"analyzer", r"analysis", r"analyze"],
    "code_quality": [r"code_quality", r"quality", r"lint"],
    "context": [r"context", r"codebase_context"],
    "dependencies": [r"dependenc", r"import"],
    "issues": [r"issue", r"error"],
    "visualization": [r"visual", r"display", r"render"],
}
...
def analyze_imports(file_path: str) -> set[str]:
    ...
    import_patterns = [r"import\s+([a-zA-Z0-9_\.]+)", r"from\s+([a-zA-Z0-9_\.]+)\s+import"]
...
def organize_files(directory: str, dry_run: bool = True) -> dict[str, list[str]]:
    ...
    py_files = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(".py") and os.path.isfile(os.path.join(directory, f))]
