...
def apply_patch(git_repo: Repository, patch: str):
    ...
    status = Status(
        f"[cyan]Applying patch to {total_files} files...[/cyan]", spinner="dots"
    )
    status.start()
    ...
    for idx, patch_file in enumerate(diff_patch, 1):
        try:
            status.update(
                f"[cyan]Applying patch [{idx}/{total_files}]: {patch_file.delta.new_file.path}[/cyan]"
            )
            ...
            error_count += 1
            rich.print(
                f"[red]✗[/red] Error applying patch to {patch_file.delta.new_file.path}: {e!s}"
            )
    ...
    if error_count == 0:
        rich.print(
            f"[green]✓ Successfully applied patch to all {total_files} files[/green]"
        )
    else:
        rich.print(
            f"[yellow]⚠ Applied patch with {error_count} error{'s' if error_count > 1 else ''} out of {total_files} files[/yellow]"
        )
