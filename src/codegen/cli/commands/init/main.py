...
@click.option(
    "--path",
    type=str,
    help="Path within a git repository. Defaults to the current directory.",
)
@click.option(
    "--token",
    type=str,
    help="Access token for the git repository. Required for full functionality.",
)
@click.option(
    "--language",
...
@click.option(
    "--fetch-docs", is_flag=True, help="Fetch docs and examples (requires auth)"
)
def init_command(
    path: str | None = None,
    token: str | None = None,
    ...
    codegen_dir, docs_dir, examples_dir = initialize_codegen(
        status=action, session=session, fetch_docs=fetch_docs
    )
    ...
    rich.print(
        format_command('codegen create my-function -d "describe what you want to do"')
    )
    rich.print("2. Run it:")
