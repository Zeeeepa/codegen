...
class TSDeclassify(ExternalProcess):
    def __init__(
        self, repo_path: str, base_path: str, working_dir: str = "/tmp/ts_declassify"
    ):
    ...
    def _start(self):
        try:
            ...
            try:
                ...
                logger.exception(
                    f"NPM FAIL: npm init failed with exit code {e.returncode}"
                )
    ...
    def declassify(
        self, source: str, filename: str = "file.tsx", error_on_failure: bool = True
    ):
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["npx", "codemod", "--plugin", "react-declassify", source_file],
                    cwd=self.working_dir,
                    check=True,
                    capture_output=True,
                    text=True,
                ...
                logger.exception(
                    f"DECLASSIFY FAIL: declassify failed with exit code {e.returncode}"
                )
            ...
            if (
                error_on_failure
                and "Cannot perform transformation" in declassified_source
            ):
