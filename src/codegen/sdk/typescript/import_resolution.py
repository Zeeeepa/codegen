...
class TSImport(Import["TSFile"], Exportable):
    ...
    def is_type_import(self) -> bool:
        ...
        if self.ts_node.type == "import_statement":
            ...
            while call_node.parent and call_node.parent.type in [
                "await_expression",
    ...
    def imported_exports(self) -> list[Exportable]:
        ...
        if not self.is_module_import():
            return (
                []
                if self.imported_symbol.export is None
                else [self.imported_symbol.export]
            )
    ...
    def resolved_symbol(self) -> Symbol | ExternalModule | TSFile | None:
        ...
        if self.is_default_import():
            if (
                ...
                and resolved_symbol.node_type == NodeType.FILE
            ):
                file = resolved_symbol
                if (
                    len(file.default_exports) == 1
                    and (
                        export_symbol := file.default_exports[0]
        ...
        if (
            resolved_symbol
            and resolved_symbol.node_type == NodeType.FILE
            and not self.is_module_import()
        ):
            # Perform BFS search on the file's module imports to find the resolved symbol
            module_imps_seen = set()
            module_imports_to_search = deque(
                [imp for imp in resolved_symbol.imports if imp.is_module_import()]
            )
            while module_imports_to_search:
                ...
                for export in module_imp.imported_exports:
                    if export.is_named_export():
                        ...
                        if (
                            self.symbol_name is not None
                            and export.name == self.symbol_name.source
                        ):
                        ...
                        if (
                            isinstance(exported_symbol, TSImport)
                            and exported_symbol.is_module_import()
                        ):
    ...
    def resolve_import(
        ...
        try:
            ...
            import_source = (
                self.module.source.strip('"').strip("'") if self.module else ""
            )
            ...
            if "." not in import_name:
                ...
                for p_path in possible_paths:
                    if self.ctx.to_absolute(
                        os.path.join(import_source, p_path)
                    ).exists():
            ...
            for import_source_base in (
                import_source,
                os.path.splitext(import_source)[0],
            ):
                    ...
                    if file := self.ctx.get_file(import_source_ext):
                        if self.is_module_import():
                            return ImportResolution(
                                from_file=file, symbol=None, imports_file=True
                            )
                        else:
                            ...
                            export_symbol = file.get_export(
                                export_name=self.symbol_name.source
                            )
                            ...
                            return ImportResolution(
                                from_file=file, symbol=export_symbol
                            )
    ...
    def _relative_to_absolute_import(self, relative_import: str) -> str:
        ...
        import_dir = os.path.dirname(
            import_file_path
        )  # the directory of the file this import is in
        absolute_import = os.path.join(
            import_dir, relative_import
        )  # absolute path of the import
        normalized_absolute_import = os.path.normpath(
            absolute_import
        )  # normalized absolute path of the import. removes redundant separators and './' or '../' segments.
        return normalized_absolute_import

    @classmethod
    @noapidoc
    def from_export_statement(
        cls,
        source_node: TSNode,
        file_node_id: NodeId,
        ctx: CodebaseContext,
        ...
        if export_clause := next(
            ...
            for export_specifier in export_clause.named_children:
                ...
                import_type = (
                    ImportType.DEFAULT_EXPORT
                    if (name and name.text.decode("utf-8") == "default")
                    else ImportType.NAMED_EXPORT
                )
                imp = cls(
    ...
    def from_import_statement(
        ...
        import_clause = next(
            (
                x
                for x in import_statement_node.named_children
                if x.type == "import_clause"
            ),
            None,
        )
        ...
        for import_type_node in import_clause.named_children:
            if import_type_node.type == "identifier":
                ...
                for import_specifier in import_type_node.named_children:
                    ...
                    alias_node = (
                        import_specifier.child_by_field_name("alias") or name_node
                    )
                    imp = cls(
                        ts_node=import_statement_node,
                        file_node_id=file_node_id,
                ...
                alias_node = next(
                    x for x in import_type_node.named_children if x.type == "identifier"
                )
                imp = cls(
                    ts_node=import_statement_node,
                    file_node_id=file_node_id,
    ...
    @classmethod
    @noapidoc
    def from_dynamic_import_statement(
        cls,
        import_call_node: TSNode,
        module_node: TSNode,
        file_node_id: NodeId,
        ctx: CodebaseContext,
        ...
        if import_statement_node.type == "expression_statement":
            ...
            if import_statement_node.type == "member_expression":
                ...
                import_statement_node = (
                    find_first_ancestor(
                        import_statement_node, parent_type_names, max_depth=2
                    )
                    or import_statement_node
                )
            else:
                name_type_name = (
                    "left"
                    if import_statement_node.type == "assignment_expression"
                    else "name"
                )
            ...
            if name_node is None:
                alias_node = import_statement_node.child_by_field_name(
                    "name"
                ) or import_statement_node.child_by_field_name("left")
                imp = cls(
                    ts_node=import_statement_node,
                    file_node_id=file_node_id,
                    ctx=ctx,
            ...
            if import_statement_node.type == "lexical_declaration":
                ...
                statement_node = (
                    import_statement_node.parent
                    if import_statement_node.type
                    in ["variable_declarator", "assignment_expression"]
                    else import_statement_node
                )
            ...
            if name_node.type == "property_identifier":
                ...
                if import_statement_node.type in [
                    "type_alias_declaration",
                    "variable_declarator",
                ]:
                ...
                import_type = (
                    ImportType.DEFAULT_EXPORT
                    if name_node.text.decode("utf-8") == "default"
                    else ImportType.NAMED_EXPORT
                )
                imp = cls(
    ...
    def import_specifier(self) -> Editable:
        ...
        """
        import_specifiers = find_all_descendants(
            self.ts_node, {"import_specifier", "export_specifier"}
        )
        ...
        if named := next(
            iter(find_all_descendants(self.ts_node, {"identifier"})), None
        ):
    ...
    def set_import_module(self, new_module: str) -> None:
        ...
        if (new_module.startswith('"') and new_module.endswith('"')) or (
            new_module.startswith("'") and new_module.endswith("'")
        ):
