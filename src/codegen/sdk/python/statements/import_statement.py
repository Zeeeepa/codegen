...
class PyImportStatement(ImportStatement["PyFile", PyImport, PyCodeBlock]):
    ...
    def __init__(
        ...
        if ts_node.type == "import_statement":
            imports.extend(
                PyImport.from_import_statement(ts_node, file_node_id, ctx, self)
            )
        elif ts_node.type == "import_from_statement":
            imports.extend(
                PyImport.from_import_from_statement(ts_node, file_node_id, ctx, self)
            )
        elif ts_node.type == "future_import_statement":
            imports.extend(
                PyImport.from_future_import_statement(ts_node, file_node_id, ctx, self)
            )
