...
class Symbol(Usable[Statement["CodeBlock[Parent, ...]"]], Generic[Parent, TCodeBlock]):
    ...
    def _execute_post_move_correction_strategy(self,
            ...
            is_used_in_file = any(
            usage.file == self.file and usage.node_type == NodeType.SYMBOL and usage not in encountered_symbols
    ...
    def _move_to_file(
        ...
        should_export = False
        if hasattr(self, "is_exported") and (self.is_exported or [
                usage for usage in self.usages
                if usage.usage_symbol
                not in encountered_symbols and not self.transaction_manager.get_transaction_containing_range(
                    usage.usage_symbol.file.path,usage.usage_symbol.start_byte,usage.usage_symbol.end_byte,TransactionPriority.Remove
                )
            ]):
            should_export=True
        ...
        if cleanup_unused_imports:
            for dep in self.dependencies:
                ...
                if isinstance(dep,Import):
                    ...
                    usages_in_file = [
                        symb for symb in other_usages
                            if symb.file==self.file and not self.transaction_manager.get_transaction_containing_range(
                                symb.file.path,symb.start_byte,symb.end_byte,TransactionPriority.Remove
                            )
                        ]
                    if self.transaction_manager.get_transaction_containing_range(
                        dep.file.path,dep.start_byte,dep.end_byte,transaction_order=TransactionPriority.Remove):
                        if not usages_in_file and strategy!="add_back_edge":
                            ...
                            if imp_list :=[import_str for import_str in self.file._pending_imports if dep.name and dep.name in import_str]:
                                if insert_import_list := [
                                        transaction for transaction in self.transaction_manager.queued_transactions[self.file.path]
