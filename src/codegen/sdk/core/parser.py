...
class Parser(Generic[Expression]):
    ...
    def _process_type(
        self, expr_type: type[Type] | dict[str, type[Type]], node: TSNode
    ) -> tuple[type[Type], TSNode]:
        if isinstance(expr_type, dict):
            ...
            if node.type not in self._uncovered_nodes:
                self.log(
                    f"Cannot handle nested type {node.type}, {expr_type}, {node.named_children}"
                )
    ...
    @classmethod
    def from_node_classes(
        cls, node_classes: NodeClasses, log_parse_warnings: bool = False
    ) -> Self:
        return cls(
            symbol_map=node_classes.symbol_map,
            expressions=node_classes.expression_map,
            types=node_classes.type_map,
            type_node=node_classes.type_node_type,
            _should_log=log_parse_warnings,
        )
    ...
    def parse_expression(
        ...
        **kwargs,
        ...
        if default == Value:
            if previous := parent.file._range_index.get_canonical_for_range(
                node.range, node.kind_id
            ):
                return previous
    ...
    def log_unparsed(self, node: TSNode) -> None:
        if (
            self._should_log
            and node.is_named
            and node.type not in self._uncovered_nodes
        ):
            self._uncovered_nodes.add(node.type)
            self.log(
                f"Encountered unimplemented node {node.type} with text {node.text.decode('utf-8')}"
            )
    ...
    def parse_ts_statements(
        ...
        from codegen.sdk.typescript.statements.for_loop_statement import (
            TSForLoopStatement,
        )
        from codegen.sdk.typescript.statements.if_block_statement import (
            TSIfBlockStatement,
        )
        from codegen.sdk.typescript.statements.import_statement import TSImportStatement
        from codegen.sdk.typescript.statements.labeled_statement import (
            TSLabeledStatement,
        )
        from codegen.sdk.typescript.statements.switch_statement import TSSwitchStatement
        from codegen.sdk.typescript.statements.try_catch_statement import (
            TSTryCatchStatement,
        )
        ...
        if node.type in self.expressions or node.type == "expression_statement":
            return [
                ExpressionStatement(
                    node, file_node_id, ctx, parent, 0, expression_node=node
                )
            ]
        ...
        for child in node.named_children:
            ...
            if child.type in _VALID_TYPE_NAMES:
                statements.append(
                    SymbolStatement(child, file_node_id, ctx, parent, len(statements))
                )
            elif child.type == "import_statement":
                statements.append(
                    TSImportStatement(child, file_node_id, ctx, parent, len(statements))
                )
            # =====[ Classes ]=====
            ...
            # =====[ Type Alias Declarations ]=====
                ...
                if import_node := find_import_node(child):
                    ...
                    statements.append(
                        SymbolStatement(
                            child, file_node_id, ctx, parent, len(statements)
                        )
                    )
            ...
            # =====[ Exports ]=====
                ...
                or child.text.decode("utf-8") == "export *;"
            ):
                statements.append(
                    ExportStatement(child, file_node_id, ctx, parent, len(statements))
                )

            # =====[ Non-symbol statements ] =====
            elif child.type == "comment":
                statements.append(
                    TSComment.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "return_statement":
                statements.append(
                    ReturnStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "if_statement":
                statements.append(
                    TSIfBlockStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
            elif child.type in ["while_statement", "do_statement"]:
                statements.append(
                    TSWhileStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type in ["for_statement", "for_in_statement"]:
                statements.append(
                    TSForLoopStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
            elif child.type == "try_statement":
                statements.append(
                    TSTryCatchStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
            elif child.type == "switch_statement":
                statements.append(
                    TSSwitchStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
                ...
                statements.append(
                    TSAttribute(child, file_node_id, ctx, parent, pos=len(statements))
                )
                ...
                for var in child.named_children:
                    if var.type == "string":
                        statements.append(
                            TSComment.from_code_block(var, parent, pos=len(statements))
                        )
                    elif var.type in [
                ...
                statements.append(
                    Statement.from_code_block(child, parent, pos=len(statements))
                )
                statements[-1].nested_code_blocks

        return statements

    def parse_py_statements(
        self,
        node: TSNode,
        file_node_id: NodeId,
        ctx: CodebaseContext,
        ...
        from codegen.sdk.core.statements.return_statement import ReturnStatement
        from codegen.sdk.core.statements.statement import Statement
        from codegen.sdk.python.statements.assignment_statement import (
        ...
        from codegen.sdk.python.statements.try_catch_statement import (
            PyTryCatchStatement,
        )
        ...
        for comment in top_comments:
            statements.append(
                PyComment.from_code_block(comment, parent, pos=len(statements))
            )
        ...
        for child in node.named_children:
            ...
            # =====[ Imports ] =====
                ...
                statements.append(
                    PyImportStatement(child, file_node_id, ctx, parent, len(statements))
                )
            # =====[ Non-symbol statements ] =====
            elif child.type == "comment":
                statements.append(
                    PyComment.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "raise_statement":
                statements.append(
                    RaiseStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "return_statement":
                statements.append(
                    ReturnStatement.from_code_block(child, parent, pos=len(statements))
                ...
                statements.append(
                    PyIfBlockStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
            elif child.type == "with_statement":
                statements.append(
                    WithStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "pass_statement":
                statements.append(
                    PyPassStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "break_statement":
                statements.append(
                    PyBreakStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "while_statement":
                statements.append(
                    PyWhileStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "for_statement":
                statements.append(
                    PyForLoopStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
            elif child.type == "match_statement":
                statements.append(
                    PyMatchStatement.from_code_block(child, parent, pos=len(statements))
                )
            elif child.type == "try_statement":
                statements.append(
                    PyTryCatchStatement.from_code_block(
                        child, parent, pos=len(statements)
                    )
                )
            elif child.type == "expression_statement":
                for var in child.named_children:
                    if var.type == "string":
                        statements.append(
                            PyComment.from_code_block(var, parent, pos=len(statements))
                        )
                        ...
                        if isinstance(parent.parent, Class):
                            ...
                            statements.append(
                                PyAssignmentStatement.from_assignment(
                                    child,
                                    file_node_id,
                                    ctx,
                                    parent,
    ...
    def report(self):
        if self._uncovered_nodes:
            self._console.print(
                f"Encountered unimplemented nodes {self._uncovered_nodes}"
            )
