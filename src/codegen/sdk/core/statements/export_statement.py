...
class ExportStatement(Statement["TSCodeBlock"], Generic[TExport]):
    ...
    def __init__(
        ...
        self.exports = Collection(
            export_node, self.file_node_id, self.ctx, self, bracket_size=2
        )
        if declaration := ts_node.child_by_field_name("declaration"):
            exports = TSExport.from_export_statement_with_declaration(
                ts_node, declaration, file_node_id, ctx, self, pos
            )
        elif value := ts_node.child_by_field_name("value"):
            exports = TSExport.from_export_statement_with_value(
                self.ts_node, value, self.file_node_id, self.ctx, self, self.index
            )
        else:
            exports = []
            if source_node := ts_node.child_by_field_name("source"):
                ...
                import_statement = TSImportStatement(
                    ts_node, file_node_id, ctx, parent, pos, source_node=source_node
                )
                ...
                for export_specifier in export_clause.named_children:
                    ...
                    alias_node = (
                        export_specifier.child_by_field_name("alias") or name_node
                    )
                    export = TSExport(
                        ts_node=export_specifier,
                        file_node_id=file_node_id,
                        ctx=ctx,
                        name_node=alias_node,
                        exported_symbol=name_node,
                        parent=self.exports,
                    )
                    exports.append(export)
            else:
                ...
                exports.extend(
                    TSExport.from_export_statement_with_value(
                        self.ts_node,
                        ts_node.named_children[0],
                        self.file_node_id,
                        self.ctx,
                        self,
                        self.index,
