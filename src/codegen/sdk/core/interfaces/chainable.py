...
class Chainable(Editable[Parent], Generic[Parent]):
    ...
    def with_resolution(
        self,
        resolution: ResolutionStack["Self"],
        *args,
        generic_parameters: list | None = None,
        ...
        **kwargs,
        ...
        if generic_parameters:
            if (
                isinstance(resolution.top.node, SupportsGenerics)
                and self.ctx.config.generics
            ):
                generics = {
                    k: v
                    for v, k in zip(generic_parameters, resolution.top.node.generics)
                }
            elif not generics:
    ...
    def with_resolution_frame(
        ...
        **kwargs,
            ...
            if not child._resolving:
                ...
                if len(resolved) > 0:
                    for resolution in resolved:
                        yield from self.with_resolution(
                            resolution,
                            *args,
        ...
        if generics is None:
            generics = (
                {i: v for i, v in enumerate(generic_parameters)}
                if generic_parameters
                else None
            )
        yield ResolutionStack(child).with_frame(
            self, *args, **kwargs, generics=generics
        )
