...
class Parents(Collection["TType", Parent], Generic[TType, Parent]):
    ...
    def compute_superclass_dependencies(self) -> None:
        ...
        for superclass in self:
            ...
            if len(resolution) == 1 and self.ctx.has_node(
                getattr(resolution[0], "node_id", None)
            ):
                self.ctx.add_edge(
                    dest.node_id, resolution[0].node_id, type=EdgeType.SUBCLASS
                )
            else:
                self._log_parse(
                    "%r is ambiguous with possibilities: %r.", superclass, resolution
                )
        self.parent.__dict__.pop("superclasses", None)
        self.parent.__dict__.pop("constructor", None)

    def _compute_dependencies(
        self, usage_type: UsageKind = UsageKind.SUBCLASS, dest: HasName | None = None
    ) -> None:
        if dest is None:
            dest = self.parent
        for superclass in self:
    ...
    @reader
    def is_subclass_of(
        self, parent: str | HasName, max_depth: int | None = None
    ) -> bool:
        ...
        if to_search in (c.source.split(".")[-1] for c in self.parent_class_names):
            return True
        for parent_class in self.parent._get_superclasses(
            max_depth=(max_depth if max_depth is None else max_depth - 1)
        ):
            if isinstance(parent_class, Class):
                if to_search in (
                    c.source.split(".")[-1] for c in parent_class.parent_class_names
                ):
                    return True
            if (
                isinstance(parent_class, Interface)
                and parent_class.parent_interfaces is not None
            ):
    ...
    def parent_class_names(self) -> list[Name | ChainedAttribute]:
        ...
        return [
            superclass.get_name()
            for superclass in self._symbols
            if isinstance(superclass, HasName)
        ]
