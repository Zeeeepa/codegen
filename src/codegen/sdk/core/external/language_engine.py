...
def get_language_engine(
    ...
    from codegen.sdk.typescript.external.ts_analyzer_engine import (
        NodeTypescriptEngine,
        V8TypescriptEngine,
    )
    ...
    if language == ProgrammingLanguage.TYPESCRIPT:
        if use_ts and use_v8:
            ...
            return V8TypescriptEngine(
                repo_path=codebase_context.repo_path,
                base_path=codebase_context.projects[0].base_path,
                dependency_manager=codebase_context.dependency_manager,
            )
        elif use_ts:
            # Enabled with only ts_language_engine feature flag is on
            return NodeTypescriptEngine(
                repo_path=codebase_context.repo_path,
                base_path=codebase_context.projects[0].base_path,
                dependency_manager=codebase_context.dependency_manager,
            )
