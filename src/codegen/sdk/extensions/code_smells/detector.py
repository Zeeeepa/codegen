...
class CodeSmellDetector:
    ...
    def detect_in_file(self, file: SourceFile) -> List[CodeSmell]:
        ...
        return [
            ...
            if (
                isinstance(smell.symbol, File) and smell.symbol == file
            ) or (
                hasattr(smell.symbol, "file") and smell.symbol.file == file
            )
        ]
        ...
        for function in self.codebase.functions:
            ...
            if line_count > self.config.long_function_lines:
                ...
                if complexity > self.config.long_function_complexity_threshold:
                    smell.refactoring_suggestions.append(
        ...
        for function in self.codebase.functions:
            ...
            if param_count > self.config.long_parameter_list_threshold:
                ...
                if self.codebase.language == ProgrammingLanguage.PYTHON:
                    smell.refactoring_suggestions.append(
                        "Use a dataclass or named tuple to group related parameters"
                    )
                elif self.codebase.language in (ProgrammingLanguage.TYPESCRIPT, ProgrammingLanguage.JAVASCRIPT):
                    smell.refactoring_suggestions.append(
                        "Use an options object pattern to group related parameters"
                    )
                
                smell.refactoring_suggestions.append(
                    "Consider if the function is doing too much and should be split"
                )
    ...
    def _detect_duplicate_code(self) -> List[CodeSmell]:
        ...
        for normalized_block, locations in block_map.items():
            ...
            if self.codebase.language == ProgrammingLanguage.PYTHON:
                ...
                smell.refactoring_suggestions.append(
                    "Extract duplicated code into a shared utility function"
                )
    ...
    def _detect_dead_code(self) -> List[CodeSmell]:
        ...
        for symbol in self.codebase._symbols():
            ...
            if not symbol.usages:
                ...
                # Add refactoring suggestions
                smell.refactoring_suggestions.append(
                    f"Remove unused {symbol.__class__.__name__.lower()} '{symbol.name}'"
                )
                
                if isinstance(symbol, Function) and symbol.is_public:
                    smell.refactoring_suggestions.append(
                        "If this is part of a public API, document it clearly or mark as deprecated"
                    )
    ...
    def _detect_complex_conditionals(self) -> List[CodeSmell]:
        ...
        for function in self.codebase.functions:
            ...
            if (nesting_depth > self.config.complex_conditional_depth_threshold or
                boolean_operators > self.config.complex_conditional_operators_threshold):
                ...
                # Add refactoring suggestions
                smell.refactoring_suggestions.append(
                    "Extract complex conditions into well-named predicate methods"
                )
                smell.refactoring_suggestions.append(
    ...
    def _detect_data_clumps(self) -> List[CodeSmell]:
        ...
        candidate_fields = {field for field, classes in field_to_classes.items() 
        ...
        for cls in self.codebase.classes:
            ...
            cls_fields = {field for field in candidate_fields 
                         if cls in field_to_classes[field]}
        ...
        for field_group, classes in field_groups.items():
            ...
            if self.codebase.language == ProgrammingLanguage.PYTHON:
                smell.refactoring_suggestions.append(
                    f"Extract fields {', '.join(field_group)} into a new class"
                )
                smell.refactoring_suggestions.append(
                    "Consider using composition instead of duplicating these fields"
                )
            elif self.codebase.language in (ProgrammingLanguage.TYPESCRIPT, ProgrammingLanguage.JAVASCRIPT):
                smell.refactoring_suggestions.append(
                    f"Extract fields {', '.join(field_group)} into a new interface or type"
                )
                smell.refactoring_suggestions.append(
                    "Use composition to share this data structure between classes"
                )
