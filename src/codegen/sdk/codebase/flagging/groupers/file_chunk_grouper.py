...
from codegen.shared.string.csv_utils import (
...
class FileChunkGrouper(BaseGrouper):
    ...
    def create_all_groups(
        ...
        for idx, chunk in enumerate(chunks):
            ...
            groups.append(
                Group(
                    id=idx,
                    group_by=GroupBy.FILE_CHUNK,
                    segment=list_to_comma_separated(chunk),
                    flags=chunk_flags,
                )
            )
        return groups
    ...
    def create_single_group(
        ...
        if len(all_segment_flags) == 0:
            logger.warning(
                f"🤷‍♀️ No flags found for FILE_CHUNK segment: {segment_filepaths}"
            )
        return Group(
            group_by=GroupBy.FILE_CHUNK, segment=segment, flags=all_segment_flags
        )
