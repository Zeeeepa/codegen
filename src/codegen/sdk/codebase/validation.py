...
def post_init_validation(codebase: CodebaseType) -> PostInitValidationStatus:
    ...
    num_resolved_imports = len(
        [
            ...
            and imp.imported_symbol.node_type != NodeType.EXTERNAL
        ]
    )
    if len(codebase.imports) > 0 and num_resolved_imports / len(codebase.imports) < 0.2:
        logger.info(
            f"Codebase {codebase.repo_path} has {num_resolved_imports / len(codebase.imports)} < 0.2 resolved imports"
        )
        return PostInitValidationStatus.LOW_IMPORT_RESOLUTION_RATE
    return PostInitValidationStatus.SUCCESS


def post_reset_validation(
    init_nodes,
    nodes,
    init_edges,
    edges,
...
def post_sync_validation(codebase: CodebaseType) -> bool:
    ...
    if (
        len(codebase.ctx.all_syncs) > 0
        or len(codebase.ctx.pending_syncs) > 0
        or len(codebase.ctx.transaction_manager.to_commit()) > 0
    ):
    ...
    if not codebase.ctx.config.codebase.track_graph:
        ...
        raise NotImplementedError(msg)
    return len(dict.fromkeys(codebase.ctx.old_graph.nodes())) == len(
        dict.fromkeys(codebase.ctx.nodes)
    ) and len(dict.fromkeys(codebase.ctx.old_graph.weighted_edge_list())) == len(
...
def get_edges_error(edges, init_edges):
    ...
    if extra_edges:
        extras = tabulate(
            (
                map(functools.partial(truncate_line, max_chars=50), edge)
...
"""
    ...
    if len(edges) != len(set_init_edges):
        ...
        extras = tabulate(
            (
                (*map(functools.partial(truncate_line, max_chars=50), edge), count)
                for edge, count in Counter(edges).most_common(5)
            ),
            ["Start", "End", "Edge", "Count"],
            maxcolwidths=50,
        )
        message += extras
    return message
...
"""
    for node in set_nodes - set_init_nodes:
        ...
        if isinstance(node, ExternalModule):
            message += "External Module persisted with following dependencies: " + str(
                list(
                    (node.ctx.get_node(source), edge)
                    for source, _, edge in node.ctx.in_edges(node.node_id)
                )
            )
    return message
