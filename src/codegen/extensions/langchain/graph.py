...
def _get_prompt_runnable(prompt: Optional[Prompt]) -> Runnable:
    ...
    if prompt is None:
        prompt_runnable = RunnableCallable(lambda state: state["messages"], name=PROMPT_RUNNABLE_NAME)
...
def _convert_modifier_to_prompt(func: F) -> F:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        if sum(p is not None for p in (prompt, state_modifier)) > 1:
            msg = "Expected only one of (prompt, state_modifier), got multiple values"
            raise ValueError(msg)
...
def _should_bind_tools(model: LanguageModelLike, tools: Sequence[BaseTool]) -> bool:
    ...
    if len(tools) != len(bound_tools):
        msg = "Number of tools in the model.bind_tools() and tools passed to create_react_agent must match"
        raise ValueError(msg)
    ...
    if missing_tools := tool_names - bound_tool_names:
        msg = f"Missing tools '{missing_tools}' in the model.bind_tools()"
        raise ValueError(msg)
...
def _get_model(model: LanguageModelLike) -> BaseChatModel:
    ...
    if not isinstance(model, BaseChatModel):
        msg = f"Expected `model` to be a ChatModel or RunnableBinding (e.g. model.bind_tools(...)), got {type(model)}"
        raise TypeError(msg)
...
def _validate_chat_history(
    ...
    all_tool_calls = [tool_call for message in messages if isinstance(message, AIMessage) for tool_call in message.tool_calls]
    tool_call_ids_with_results = {message.tool_call_id for message in messages if isinstance(message, ToolMessage)}
    tool_calls_without_results = [tool_call for tool_call in all_tool_calls if tool_call["id"] not in tool_call_ids_with_results]
...
def create_react_agent(
    ...
    *,
    ...
    Examples:
        ...
        I like to bike
        ================================== Ai Message ==================================
        That's great to hear, Will! Biking is an excellent hobby and form of exercise. It's a fun way to stay active and explore your surroundings.
        Do you have any favorite biking routes or trails you enjoy? Or perhaps you're into a specific type of biking, like mountain biking or road cycling?
        ...
        ================================== Ai Message ==================================
        User memories:
        Hello! Of course, I remember you, Will! You mentioned earlier that you like to bike.
        It's great to hear from you again. How have you been? Have you been on any interesting bike rides lately?
        ```
    ...
    if version not in ("v1", "v2"):
        msg = f"Invalid version {version}. Supported versions are 'v1' and 'v2'."
        raise ValueError(msg)
    ...
    def call_model(state: AgentState, config: RunnableConfig) -> AgentState:
        ...
        all_tools_return_direct = all(call["name"] in should_return_direct for call in response.tool_calls) if isinstance(response, AIMessage) else False
        if (
            ("remaining_steps" not in state and state.get("is_last_step", False) and has_tool_calls)
    ...
    def generate_structured_response(state: AgentState, config: RunnableConfig) -> AgentState:
        ...
        model_with_structured_output = _get_model(model).with_structured_output(cast(StructuredResponseSchema, structured_response_schema))
        response = model_with_structured_output.invoke(messages, config)
        return {"structured_response": response}

    async def agenerate_structured_response(state: AgentState, config: RunnableConfig) -> AgentState:
        # NOTE: we exclude the last message because there is enough information
        # for the LLM to generate the structured response
        messages = state["messages"][:-1]
        structured_response_schema = response_format
    ...
    workflow.add_node("agent", RunnableCallable(call_model, acall_model), retry=retry_policy)
