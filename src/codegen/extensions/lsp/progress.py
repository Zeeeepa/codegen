...
class LSPTask(Task):
    ...
    def __init__(
        ...
        if create_token:
            server.work_done_progress.begin(
                self.token, types.WorkDoneProgressBegin(title=message)
            )
        self.server = server
    ...
    def update(self, message: str, count: int | None = None) -> None:
        ...
        self.server.work_done_progress.report(
            self.token,
            types.WorkDoneProgressReport(message=message, percentage=percent),
        )

    def end(self) -> None:
        if self.create_token:
            self.server.work_done_progress.end(
                self.token, value=types.WorkDoneProgressEnd()
            )
...
class LSPProgress(Progress[LSPTask | StubTask]):
    ...
    def __init__(
        self, server: LanguageServer, initial_token: ProgressToken | None = None
    ):
        self.server = server
        self.initial_token = initial_token
        if initial_token is not None:
            self.server.work_done_progress.begin(
                initial_token, types.WorkDoneProgressBegin(title="Parsing codebase...")
            )

    def begin_with_token(
        self,
        message: str,
        token: ProgressToken | None = None,
        *,
        count: int | None = None,
        create_token: bool = True,
    ) -> LSPTask | StubTask:
    ...
    def begin(self, message: str, count: int | None = None) -> LSPTask | StubTask:
        ...
        return self.begin_with_token(
            message, self.initial_token, count=None, create_token=False
        )
    ...
    def finish_initialization(self) -> None:
        ...
        if self.initial_token is not None:
            self.server.work_done_progress.end(
                self.initial_token, value=types.WorkDoneProgressEnd()
            )
