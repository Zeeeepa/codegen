...
from codegen.extensions.graph.utils import (
    Node,
    NodeLabel,
    Relation,
    RelationLabel,
...
def create_codebase_graph(codebase):
    ...
    def get_or_create_node(
        ...
        full_name = (
            f"{parent_name}.{name}" if parent_name and parent_name != "Class" else name
        )
    ...
    def create_class_node(class_def):
        ...
        return get_or_create_node(
            ...
            properties={
                "filepath": (
                    class_def.filepath if hasattr(class_def, "filepath") else ""
                ),
    ...
    for class_def in codebase.classes:
        ...
        for method in methods:
            ...
            defines_relation = Relation(
                label=RelationLabel.DEFINES.value,
                source_id=class_node.id,
                target_id=method_node.id,
                properties={
                    "relationship_description": "The parent class defines the method."
                },
            )
            ...
            for call in method.function_calls:
                ...
                if call_node and call_node != method_node:
                    call_relation = Relation(
                        label=RelationLabel.CALLS.value,
                        source_id=method_node.id,
                        target_id=call_node.id,
                        properties={
                            "relationship_description": f"The method calls the {call_node.label}."
                        },
                    )
        ...
        if class_def.parent_classes:
            for parent in class_def.parent_classes:
                ...
                inherits_relation = Relation(
                    ...
                    properties={
                        "relationship_description": "The child class inherits from the parent class."
                    },
                )
        ...
        for call in func.function_calls:
            ...
            if call_node and call_node != func_node:
                call_relation = Relation(
                    ...
                    target_id=call_node.id,
                    properties={
                        "relationship_description": f"The function calls the {call_node.label}."
                    },
                )
                graph.add_relation(call_relation)
