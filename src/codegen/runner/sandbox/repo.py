...
class SandboxRepo:
    ...
    def set_up_base_branch(self, base_branch: str | None) -> None:
        ...
        self.codebase.op.fetch_remote(
            highside_remote.name, refspec=f"{base_branch}:{base_branch}"
        )
    ...
    def set_up_head_branch(self, head_branch: str, force_push_head_branch: bool):
        ...
        highside_remote = self.codebase.op.git_cli.remote(name="origin")
        self.codebase.op.fetch_remote(
            highside_remote.name, refspec=f"{head_branch}:{head_branch}"
        )
    ...
    def push_changes_to_remote(
        self, commit_msg: str, head_branch: str, force_push: bool
    ) -> bool:
        ...
        if not has_staged_commit:
            logger.info(
                "Skipping opening pull request for cm_run b/c the codemod produced no changes"
            )
            return False

        # =====[ Push changes highside ]=====
        highside_remote = self.codebase.op.git_cli.remote(name="origin")
        highside_res = self.codebase.op.push_changes(
            remote=highside_remote,
            refspec=f"{head_branch}:{head_branch}",
            force=force_push,
        )
