...
class SandboxRunner:
    ...
    def __init__(self, repo_config: RepoConfig, op: RepoOperator | None = None) -> None:
        ...
        self.op = op or RepoOperator(
            repo_config=self.repo,
            setup_option=SetupOption.PULL_OR_CLONE,
            bot_commit=True,
        )
    ...
    async def warmup(self, codebase_config: CodebaseConfig | None = None) -> None:
        ...
        logger.info(
            f"===== Warming runner for {self.repo.full_name or self.repo.name} ====="
        )
    ...
    async def _build_graph(
        self, codebase_config: CodebaseConfig | None = None
    ) -> Codebase:
        logger.info("> Building graph...")
        projects = [
            ProjectConfig(
                programming_language=self.repo.language,
                repo_operator=self.op,
                base_path=self.repo.base_path,
                subdirectories=self.repo.subdirectories,
            )
        ]
    ...
    async def get_diff(self, request: GetDiffRequest) -> GetDiffResponse:
        custom_scope = (
            {"context": request.codemod.codemod_context}
            if request.codemod.codemod_context
            else {}
        )
        code_to_exec = create_execute_function_from_codeblock(
            codeblock=request.codemod.user_code, custom_scope=custom_scope
        )
        session_options = SessionOptions(
            max_transactions=request.max_transactions, max_seconds=request.max_seconds
        )
    ...
    async def create_branch(self, request: CreateBranchRequest) -> CreateBranchResponse:
        ...
        branch_config.custom_base_branch = (
            branch_config.custom_base_branch or self.codebase.default_branch
        )
        self.executor.remote_repo.set_up_base_branch(branch_config.custom_base_branch)
        self.executor.remote_repo.set_up_head_branch(
            branch_config.custom_head_branch, branch_config.force_push_head_branch
        )
        ...
        if "codebase.flag_instance" in request.codemod.user_code:
            ...
            flag_groups = await self.executor.find_flag_groups(
                flags, request.grouping_config
            )
        ...
        run_results, branches = await self.executor.execute_flag_groups(
            request.commit_msg, code_to_exec, flag_groups, branch_config
        )
