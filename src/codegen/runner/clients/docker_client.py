...
class DockerClient(Client):
    ...
    def __init__(self, container: DockerContainer):
        if (
            ...
            or container.host is None
            or container.port is None
        ):
    ...
    def run(self, codemod_source: str, commit: bool | None = None) -> CodemodRunResult:
        req = RunFunctionRequest(
            function_name="unnamed", codemod_source=codemod_source, commit=commit
        )
        response = self.post(RUN_FUNCTION_ENDPOINT, req.model_dump())
        return CodemodRunResult.model_validate(response.json())

    def run_function(
        self, function: DecoratedFunction, commit: bool
    ) -> CodemodRunResult:
        req = RunFunctionRequest(
            function_name=function.name, codemod_source=function.source, commit=commit
        )
        response = self.post(RUN_FUNCTION_ENDPOINT, req.model_dump())
        return CodemodRunResult.model_validate(response.json())
...
if __name__ == "__main__":
    ...
    cur = next(
        (container for container in fleet.containers if container.is_running()), None
    )
    if cur is None:
