...
def compare_codebase_diff(
    ...
    if not snapshot._snapshot_update:
        ...
        if convert_diff_to_repo(expected_dir, expected_diff, codebase):
            return compare_codebase_with_snapshot(
                codebase, expected_dir, diff_path, snapshot, modified
            )
    # === [Snapshot Comparison] ===
...
def compare_codebase_with_snapshot(
    codebase: Codebase,
    expected_dir: Path,
    diff_path: Path,
    snapshot: Snapshot,
    ...
    if not expected_dir.exists():
        logger.warning(
            f"There is no result to compare this test case to. {expected_dir} does not exist"
        )
        return
    ...
    if not snapshot._snapshot_update and expected_dir.is_dir():
        analyze_codebase_diff(
            expected_dir, codebase_path, diff_path, extensions, capture_stats
        )
    snapshot.assert_match_dir(unflatten_to_dict(modified), expected_dir.name)


def compare_expected_actual_diffs(
    codebase: Codebase,
    actual_diff: str,
    expected_diff: str,
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp_dir:
        ...
        if convert_diff_to_repo(expected_dir, expected_diff_path, codebase):
            ...
            subprocess.run(
                ["git", "apply", str(actual_diff_path.absolute())],
...
def capture_single_file_stats(
    files_dir: Path, files, expected_dir: Path, extensions: set, diff_stats: dict
) -> list[str]:
...
def capture_modified_file_stats(
    left_lines: list[str], right_lines: list[str], diff_stats: dict
):
...
def analyze_codebase_diff(
    expected_dir: Path,
    modified_dir: Path,
    diff_path: Path,
    extensions: set[str] = {".py"},
    capture_stats: bool = False,
    single_diff_file: bool = False,
) -> dict | None:
    ...
    def process_diff(
        ...
        for filename in common_files:
            ...
            if not filecmp.cmp(left_file, right_file, shallow=False):
                ...
                if single_diff_file:
                    ...
                    resulting_diff += (
                        f"<h2>Diff for {file_path!s}</h2>" + "\n" + html_diff + "\n"
                    )
                else:
                    (diff_path / str(relative_path)).with_suffix(".html").parent.mkdir(
                        parents=True, exist_ok=True
                    )
                    (diff_path / str(relative_path)).with_suffix(".html").write_text(
                        html_diff
                    )
        ...
        if capture_stats:
            left_diff = capture_single_file_stats(
                left_dir, left_only, expected_dir, extensions, diff_stats
            )
            right_diff = capture_single_file_stats(
                right_dir, right_only, modified_dir, extensions, diff_stats
            )
        ...
        for subdir, subdir_dcmp in subdirs.items():
            resulting_diff = process_diff(
                left_dir / subdir,
                right_dir / subdir,
                subdir_dcmp.common_files,
                subdir_dcmp.left_only,
                subdir_dcmp.right_only,
                subdir_dcmp.subdirs,
                resulting_diff,
            )
