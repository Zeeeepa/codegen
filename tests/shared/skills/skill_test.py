...
class SkillTestCase:
    ...
    name: str | None = (
        None  # name the test case to make it easier to find + test a specific case
    )
    filepath: str = ""
    sanity: bool = (
        False  # sanity check: run the skill on the input and check if the output is the same
    )
    graph: bool = False

    @classmethod
    def from_files(
        cls, files: list[SkillTestCasePyFile | SkillTestCaseTSFile]
    ) -> "SkillTestCase":
    ...
    def from_dir(cls, filepath: Path) -> "SkillTestCase":
        ...
        original_files = set(
            f.relative_to(original_dir) for f in original_dir.glob("**/*.*")
        )
        expected_files = set(
            f.relative_to(expected_dir) for f in expected_dir.glob("**/*.*")
        )
