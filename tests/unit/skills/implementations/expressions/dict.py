...
test_cases = [
    SkillTestCase(
        files=[
            SkillTestCasePyFile(input='a={"foo": "bar"}', output="a=Schema(foo=bar)")
        ]
    ),
    ...
    SkillTestCase(
        [
            SkillTestCasePyFile(
                input='a={"foo": "bar", "baz":"hi"}', output="a=Schema(foo=bar, baz=hi)"
            )
        ]
    ),
]
...
@skill(
    prompt="Please write a codemod that converts all dictionaries to Schema objects",
    uid="6ef2ffa5-94c3-42a1-84dd-da56ea41021b",
)
    ...
    def python_skill_func(codebase: PyCodebaseType):
        ...
        for v in codebase.global_vars:
            ...
            if isinstance(v.value, Dict):
                ...
                v.set_value(
                    f"Schema({', '.join(f'{k}={v}' for k, v in v.value.items())})"
                )
