...
@pytest.mark.skip(reason="V8 engine is not yet fully supported")
def test_v8_engine(tmpdir) -> None:
    ts_config = """
{
  "compilerOptions": {
    "target": "es5",
...
"""
    with get_codebase_session(
        tmpdir=tmpdir,
        files={"tsconfig.json": ts_config, "file1.ts": content1},
        ...
        codebase._enable_experimental_language_engine(use_v8=True)

        assert (
            file.get_function("getUser").inferred_return_type
        ...
        assert (
            file.get_function("fetchUserData").inferred_return_type
            == "Promise<Response>"
        )
