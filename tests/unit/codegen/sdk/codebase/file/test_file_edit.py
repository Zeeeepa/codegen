...
def test_codebase_edit_mdx(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir,
        files={"file1.mdx": "# Header", "file2.tsx": "console.log('hello, world!')"},
    ) as codebase:
...
def test_edit_json_file(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir,
        files={"config.json": '{"key": "value", "nested": {"foo": "bar"}}'},
    ) as codebase:
...
def test_edit_txt_file(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir, files={"data.txt": "Hello\nWorld\nTest"}
    ) as codebase:
...
def test_codebase_replace_mdx(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir, files={"file1.mdx": "# Header"}
    ) as codebase:
        file = codebase.get_file("file1.mdx")
...
def test_replace_non_source_file(tmpdir) -> None:
    ...
    with get_codebase_session(
        ...
        files={
            "doc.mdx": "# Header\nThis is a test\nMore content",
            "config.json": '{"test": "value", "other": "test"}',
        },
    ) as codebase:
...
def test_edit_binary_file_fails(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir, files={"image.png": binary_content}
    ) as codebase:
...
def test_edit_source_file_preserves_behavior(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir, files={"script.py": "def test():\n    print('hello')"}
    ) as codebase:
...
def test_transaction_ordering_non_source_files(tmpdir) -> None:
    ...
    with get_codebase_session(
        tmpdir=tmpdir, files={"doc.md": "# Header\nContent\nFooter"}
    ) as codebase:
