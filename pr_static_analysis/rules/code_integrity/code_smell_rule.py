...
class CodeSmellRule(BaseCodeIntegrityRule):
    ...
    def description(self) -> str:
        ...
        return (
            "Detects common code smells, such as long functions, deeply nested code, "
            "magic numbers, duplicate code, and empty catch blocks."
        )
                    ...
                    results.append(
                        RuleResult(
                            ...
                            message=(
                                ...
                                f"({function_length} lines, max {max_length})"
                            ),
                            filepath=filepath,
                            line=node.lineno,
        ...
        if visitor.max_depth_seen > max_depth and visitor.max_depth_node:
            results.append(
                RuleResult(
                    ...
                    severity=self.severity,
                    message=(
                        f"Code is too deeply nested "
                        f"(depth {visitor.max_depth_seen}, max {max_depth})"
                    ),
                    filepath=filepath,
    ...
    def _check_empty_catch_blocks(
            ...
            def visit_ExceptHandler(self, node):
                ...
                if (
                    ...
                    or (
                        ...
                        and node.body[0].value.value in (None, "")
                    )
                ):
        ...
        for node in visitor.empty_catches:
            exception_name = (
                f"Exception {node.name}" if node.name else "Exception"
            )
            exception_type = (
                ast.get_source_segment(content, node.type) if node.type else "all exceptions"
            )
    ...
    def _check_duplicate_code(self, files: List[Dict[str, Any]]) -> List[RuleResult]:
        ...
        python_files = [
            file_info
            for file_info in files
            if file_info.get("filepath", "").endswith(".py")
            and file_info.get("content")
        ]
        
        # Simple duplicate code detection (line-based)
        ...
        for i, file1 in enumerate(python_files):
            for j in range(i + 1, len(python_files)):
                ...
                duplicates = self._find_duplicate_blocks(
                    lines1, lines2, min_duplicate_lines
                )
                ...
                for (start1, end1, start2, end2) in duplicates:
                    ...
                    results.append(
                        RuleResult(
                            ...
                            severity=self.severity,
                            message=(
                                f"Duplicate code found in {filepath1} (lines {start1+1}-{end1}) "
                                f"and {filepath2} (lines {start2+1}-{end2})"
                            ),
                            filepath=filepath1,
                            line=start1 + 1,
                            code_snippet=duplicate_code,
                            fix_suggestions=[
                                "Extract the duplicate code into a shared function",
                                "Create a utility module for shared functionality",
                                "Use inheritance or composition to share code",
                            ],
                            metadata={
                                "duplicate_filepath": filepath2,
                                "duplicate_line": start2 + 1,
                                "duplicate_length": end1 - start1,
                            },
                        )
                    )
    ...
    def _find_duplicate_blocks(
        self, lines1: List[str], lines2: List[str], min_length: int
