...
from pr_static_analysis.rules.base.base_rule import BaseRule
...
class RuleConfig:
        ...
        try:
            with open(config_file) as f:
                if config_file.endswith(".yaml") or config_file.endswith(".yml"):
                    ...
                    msg = f"Unsupported configuration file format: {config_file}"
                    raise ValueError(msg)

            self.load_config(config)

        except (yaml.YAMLError, json.JSONDecodeError) as e:
            msg = f"Error parsing configuration file '{config_file}': {e}"
            raise ValueError(msg)
    ...
    def is_rule_enabled(self, rule_id: str) -> bool:
        ...
        if rule_class:
            return bool(rule_class.enabled.__get__(None, rule_class))
        return False
    ...
    def get_enabled_rule_ids(self) -> list[str]:
        ...
        """
        return [rule_id for rule_id in rule_registry.get_all_rules().keys() if self.is_rule_enabled(rule_id)]
