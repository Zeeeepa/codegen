...
function pathUnban(org){ return `/v1/organizations/${org}/agent/run/unban`; }
...
async function main(){
  ...
  if (!id) { console.error('Usage: unban_agent_run.js --id <runId> [--before <order>] [--after <order>]'); process.exit(2); }
  ...
  const data = await apiPost(pathUnban(org), body);
